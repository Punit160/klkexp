...
function waitFor(callback, { timeOutAfter = 500, retryAfter = 100 } = {}) {
	return new Promise((resolve, reject) => {
		...
		const timeoutTimerId = setTimeout(() => {
			reject(lastError ?? /* @__PURE__ */ new Error("Timeout"));
		}, timeOutAfter);
		const tick = async () => {
			try {
...
function overwriteObject(source, destination) {
	for (const prop of Object.getOwnPropertyNames(destination)) delete destination[prop];
	for (const [key, value] of Object.entries(source)) if (value !== destination && key !== "prototype" && key !== "__proto__") destination[key] = value;
	return destination;
}
...
function createCKCdnBaseBundlePack({ version, translations, createCustomCdnUrl = createCKCdnUrl }) {
	...
	return {
		...
		scripts: [async (attributes) => injectScriptsInParallel(urls.scripts, attributes)],
...
async function loadCKCdnResourcesPack(pack) {
	...
	for (const script of uniq(scripts)) {
		...
		if (typeof script === "string") await injectScript(script, injectorProps);
		else await script(injectorProps);
	}
...
function combineCKCdnBundlesPacks(packs) {
	...
	const checkPluginLoaded = async () => {
		...
		for (const [name, pack] of Object.entries(normalizedPacks)) exportedGlobalVariables[name] = await pack?.checkPluginLoaded?.();
		return exportedGlobalVariables;
	};
	const beforeInject = () => {
		for (const pack of Object.values(normalizedPacks)) pack.beforeInject?.();
	};
	return {
		...mergedPacks,
		beforeInject,
...
function createCKBoxBundlePack({ version, theme = "lark", translations, createCustomCdnUrl = createCKBoxCdnUrl }) {
	return {
		scripts: [createCustomCdnUrl("ckbox", "ckbox.js", version), ...without(["en"], translations || []).map((translation) => createCustomCdnUrl("ckbox", `translations/${translation}.js`, version))],
		...theme && { stylesheets: [createCustomCdnUrl("ckbox", `styles/themes/${theme}.css`, version)] },
...
function combineCdnPluginsPacks(pluginsPacks) {
	return combineCKCdnBundlesPacks(mapObjectValues(pluginsPacks, (pluginPack, pluginName) => {
		...
		return {
			...
			...normalizeCKCdnResourcesPack(pluginPack)
		};
	}));
}
function loadCKEditorCloud(config) {
	...
	return loadCKCdnResourcesPack({
		...combineCKCdnBundlesPacks({
			...
			...ckbox && { CKBox: createCKBoxBundlePack(ckbox) },
...
* For licensing, see LICENSE.md or https://ckeditor.com/legal/ckeditor-licensing-options
*/
var useInitializedCKEditorsMap = ({ currentContextWatchdog, onChangeInitializedEditors }) => {
	const onChangeInitializedEditorsSafe = useRefSafeCallback(onChangeInitializedEditors || (() => {}));
	(0, import_react.useEffect)(() => {
		var _a;
		if (currentContextWatchdog.status !== "initialized") return;
		...
		const getInitializedContextEditors = () => [...editors].reduce((map, editor) => {
			var _a2;
			if (editor.state !== "ready") return map;
		...
		if (Array.from(editors).some((editor) => editor.state === "ready")) onEditorStatusChange();
		return () => {
...
var CKEditorContext = (props) => {
	...
	const [currentContextWatchdog, setCurrentContextWatchdog] = (0, import_react.useState)({ status: "initializing" });
	(0, import_react.useEffect)(() => {
		if (isLayoutReady) initializeContextWatchdog();
		else setCurrentContextWatchdog({ status: "initializing" });
	}, [id, isLayoutReady]);
	(0, import_react.useEffect)(() => () => {
		if (currentContextWatchdog.status === "initialized") currentContextWatchdog.watchdog.destroy();
	}, [currentContextWatchdog]);
	...
	function initializeContextWatchdog() {
		...
		contextWatchdog.on("error", (_, errorEvent) => {
			...
			if (canUpdateState(watchdogInitializationID)) onError(errorEvent.error, {
				phase: "runtime",
				willContextRestart: errorEvent.causesRestart
			});
		});
		contextWatchdog.on("stateChange", () => {
			if (onReady && contextWatchdog.state === "ready" && canUpdateState(watchdogInitializationID)) onReady(contextWatchdog.context, contextWatchdog);
		});
		contextWatchdog.create(config).then(() => {
			if (canUpdateState(watchdogInitializationID)) setCurrentContextWatchdog({
				status: "initialized",
				watchdog: contextWatchdog
			});
			else contextWatchdog.destroy();
		}).catch((error) => {
	...
	return /* @__PURE__ */ import_react.createElement(ContextWatchdogContext.Provider, { value: currentContextWatchdog }, children);
};
...
var CKEditor = class extends import_react.Component {
	...
	shouldComponentUpdate(nextProps) {
		...
		if (editorSemaphore) {
			editorSemaphore.runAfterMount(({ instance }) => {
				if (this._shouldUpdateEditorData(props, nextProps, instance)) instance.data.set(nextProps.data);
			});
	...
	*/
	render() {
		return /* @__PURE__ */ import_react.createElement("div", { ref: this.domContainer });
	}
	/**
	* Initializes the editor by creating a proper watchdog and initializing it with the editor's configuration.
	*/
	async _initializeEditor() {
		if (this.props.disableWatchdog) return {
		...
		const totalRestartsRef = { current: 0 };
		watchdog.setCreator(async (el, config) => {
			var _a;
			const { editorSemaphore } = this;
			const { onAfterDestroy } = this.props;
			if (totalRestartsRef.current > 0 && onAfterDestroy && ((_a = editorSemaphore == null ? void 0 : editorSemaphore.value) == null ? void 0 : _a.instance)) onAfterDestroy(editorSemaphore.value.instance);
			...
			if (editorSemaphore && totalRestartsRef.current > 0) {
				...
				setTimeout(() => {
					if (this.props.onReady) this.props.onReady(watchdog.editor);
				});
			}
	...
	*/
	_createEditor(element, config) {
		...
		if (contextItemMetadata) config = withCKEditorReactContextMetadata(contextItemMetadata, config);
		return this.props.editor.create(element, appendAllIntegrationPluginsToConfig(config)).then((editor) => {
			...
			modelDocument.on("change:data", (event) => {
				/* istanbul ignore else -- @preserve */
				if (this.props.onChange) this.props.onChange(event, editor);
			});
			viewDocument.on("focus", (event) => {
				/* istanbul ignore else -- @preserve */
				if (this.props.onFocus) this.props.onFocus(event, editor);
			});
			viewDocument.on("blur", (event) => {
				/* istanbul ignore else -- @preserve */
				if (this.props.onBlur) this.props.onBlur(event, editor);
			});
			return editor;
		});
	}
	...
	*/
	_getConfig() {
		...
		if (this.props.data && config.initialData) console.warn("Editor data should be provided either using `config.initialData` or `content` property. The config value takes precedence over `content` property and will be used when both are specified.");
		return {
			...config,
...
var useLifeCycleSemaphoreSyncRef = () => {
	...
	const release = (rerender = true) => {
		...
		if (rerender) setRevision(Date.now());
	};
	const unsafeSetValue = (value) => {
		var _a;
		(_a = semaphoreRef.current) == null || _a.unsafeSetValue(value);
		refresh();
	};
	const runAfterMount = (callback) => {
		if (semaphoreRef.current) semaphoreRef.current.runAfterMount(callback);
	};
	...
	const createAttributeRef = (key) => ({ get current() {
		if (!semaphoreRef.current || !semaphoreRef.current.value) return null;
...
var useInstantEditorEffect = (semaphore, fn, deps) => {
	useInstantEffect(() => {
		if (semaphore) semaphore.runAfterMount(fn);
	}, [semaphore, ...deps]);
};
/**
* @license Copyright (c) 2003-2025, CKSource Holding sp. z o.o. All rights reserved.
...
var useMultiRootEditor = (props) => {
	...
	const forceAssignFakeEditableElements = () => {
		...
		const initializeEditableWithFakeElement = (editable) => {
			if (editable.name && !editor.editing.view.getDomRoot(editable.name)) editor.editing.view.attachDomRoot(document.createElement("div"), editable.name);
		};
	...
	(0, import_react.useEffect)(() => {
		...
		if (context && !isContextWatchdogReadyToUse(context)) return;
		if (!semaphoreElement || props.isLayoutReady === false) return;
		semaphore.replace(() => new LifeCycleElementSemaphore(semaphoreElement, {
			mount: _initializeEditor,
			afterMount: ({ mountResult }) => {
				const { onReady } = props;
				if (onReady && semaphoreElementRef.current !== null) onReady(mountResult.instance);
			},
			unmount: async ({ element, mountResult }) => {
	...
	const _getConfig = () => {
		...
		if (props.data && config.initialData) console.warn("Editor data should be provided either using `config.initialData` or `data` property. The config value takes precedence over `data` property and will be used when both are specified.");
		return {
			...config,
	...
	const onChangeData = useRefSafeCallback((editor, event) => {
		...
		if (!props.disableTwoWayDataBinding) {
			...
			modelDocument.differ.getChanges().forEach((change) => {
				let root;
				/* istanbul ignore else -- @preserve */
				if (change.type == "insert" || change.type == "remove") root = change.position.root;
				else root = change.range.root;
			...
			modelDocument.differ.getChangedRoots().forEach((changedRoot) => {
				if (changedRoot.state) {
					if (newData[changedRoot.name] !== void 0) delete newData[changedRoot.name];
					return;
				}
				const rootName = changedRoot.name;
				newAttributes[rootName] = editor.getRootAttributes(rootName);
			});
			if (Object.keys(newData).length) setData((previousData) => ({
				...previousData,
				...newData
			}));
			if (Object.keys(newAttributes).length) setAttributes((previousAttributes) => ({
				...previousAttributes,
				...newAttributes
			}));
		}
		/* istanbul ignore else -- @preserve */
		if (props.onChange) props.onChange(event, editor);
	});
 ...
 /* istanbul ignore else -- @preserve */
			...
			viewDocument.on("focus", (event) => {
				/* istanbul ignore else -- @preserve */
				if (props.onFocus) props.onFocus(event, editor);
			});
			viewDocument.on("blur", (event) => {
				/* istanbul ignore else -- @preserve */
				if (props.onBlur) props.onBlur(event, editor);
			});
			return editor;
		});
	});
	...
	const _initializeEditor = async () => {
		if (props.disableWatchdog) return {
			instance: await _createEditor(props.data, _getConfig()),
			watchdog: null
		};
		const watchdog = (() => {
			if (isContextWatchdogReadyToUse(context)) return new EditorWatchdogAdapter(context.watchdog);
			return new props.editor.EditorWatchdog(props.editor, props.watchdogConfig);
		})();
		const totalRestartsRef = { current: 0 };
		watchdog.setCreator(async (_, config) => {
			const { onAfterDestroy } = props;
			if (totalRestartsRef.current > 0 && onAfterDestroy && editorRefs.instance.current) onAfterDestroy(editorRefs.instance.current);
			...
			if (totalRestartsRef.current > 0) {
				...
				setTimeout(() => {
					/* istanbul ignore next -- @preserve */
					if (props.onReady) props.onReady(watchdog.editor);
				});
			}
			totalRestartsRef.current++;
			return instance;
		...
		watchdog.on("error", (_, { error, causesRestart }) => {
			(props.onError || console.error)(error, {
				phase: "runtime",
				willEditorRestart: causesRestart
			});
		});
	...
	const _externalSetData = (0, import_react.useCallback)((newData) => {
		semaphore.runAfterMount(() => {
			shouldUpdateEditor.current = true;
			setData(newData);
		});
	}, [setData]);
	const _externalSetAttributes = (0, import_react.useCallback)((newAttributes) => {
		semaphore.runAfterMount(() => {
			shouldUpdateEditor.current = true;
			setAttributes(newAttributes);
		});
	}, [setAttributes]);
	const toolbarElement = /* @__PURE__ */ import_react.createElement(EditorToolbarWrapper, {
		ref: semaphoreElementRef,
		editor: editorRefs.instance.current
	});
	useInstantEditorEffect(semaphore.current, ({ instance }) => {
		if (props.disabled) instance.enableReadOnlyMode(REACT_INTEGRATION_READ_ONLY_LOCK_ID);
	...
	useInstantEditorEffect(semaphore.current, ({ instance }) => {
		if (shouldUpdateEditor.current) {
			...
			const { addedKeys: newRoots, removedKeys: removedRoots } = _getStateDiff(editorData, data || {});
			...
			setTimeout(() => {
				instance.model.change((writer) => {
					...
					if (modifiedRoots.length) _updateEditorData(modifiedRoots);
					if (rootsWithChangedAttributes.length) _updateEditorAttributes(writer, rootsWithChangedAttributes);
				});
			});
		}
	}, [data, attributes]);
	const editableElements = roots.map((rootName) => /* @__PURE__ */ import_react.createElement(EditorEditable, {
		key: rootName,
		id: rootName,
		rootName,
		semaphore
...
var useAsyncCallback = (callback) => {
	const [asyncState, setAsyncState] = (0, import_react.useState)({ status: "idle" });
	const unmountedRef = useIsUnmountedRef();
	const prevExecutionUIDRef = (0, import_react.useRef)(null);
	return [useRefSafeCallback(async (...args) => {
		if (unmountedRef.current || isSSR()) return null;
		const currentExecutionUUID = uid();
		prevExecutionUIDRef.current = currentExecutionUUID;
		try {
			if (asyncState.status !== "loading") setAsyncState({ status: "loading" });
			const result = await callback(...args);
			if (!unmountedRef.current && prevExecutionUIDRef.current === currentExecutionUUID) setAsyncState({
				status: "success",
				data: result
			});
			return result;
...
var useAsyncValue = (callback, deps) => {
	...
	if (asyncState.status === "idle") return { status: "loading" };
	return asyncState;
};
/**
* @license Copyright (c) 2003-2025, CKSource Holding sp. z o.o. All rights reserved.
...
function useCKEditorCloud(config) {
	const result = useAsyncValue(async () => loadCKEditorCloud(config), [JSON.stringify(config)]);
...
var withCKEditorCloud = (config) => (WrappedComponent) => {
	const ComponentWithCKEditorCloud = (props) => {
		...
		switch (ckeditorCloudResult.status) {
			case "error":
				if (!config.renderError) return "Unable to load CKEditor Cloud data!";
				return config.renderError(ckeditorCloudResult.error);
			case "success": return /* @__PURE__ */ import_react.createElement(WrappedComponent, {
				...props,
				cloud: ckeditorCloudResult
			});
			default: return (_b = (_a = config.renderLoader) == null ? void 0 : _a.call(config)) != null ? _b : null;
		}
	};
...
//#endregion
export { CKEditor, CKEditorContext, loadCKEditorCloud, useCKEditorCloud, useMultiRootEditor, withCKEditorCloud };
