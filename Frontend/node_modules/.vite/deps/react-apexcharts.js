...
var SSRDOMShim = class {
	...
	*/
	createDOMParser() {
		return { parseFromString(str, _type) {
	...
	init({ responsiveOverride }) {
		...
		if (opts && typeof opts === "object") {
			...
			if ([
				...
				"radar",
				"radialBar"
			].indexOf(opts.chart.type) !== -1) chartDefaults = defaults[opts.chart.type]();
...
var CoreUtils = class {
	...
	*/
	getSeriesTotalByIndex(index = null) {
		...
		else return this.w.seriesData.series[index].reduce((acc, cur) => acc + cur, 0);
	}
	/**
	* @memberof CoreUtils
	...
	*/
	isSeriesNull(index = null) {
		let r = [];
		if (index === null) r = this.w.config.series.filter((d) => d !== null);
		else r = this.w.config.series[index].data.filter((d) => d !== null);
		return r.length === 0;
	}
	/**
	* @param {number} index
...
var Graphics = class Graphics {
	...
	**/
	renderPaths({ j, realIndex, pathFrom, pathTo, stroke, strokeWidth, strokeLinecap, fill, animationDelay, initialSpeed, dataChangeSpeed, className, chartType, shouldClipToGrid = true, bindEventsOnPaths = true, drawShadow = true }) {
		const w = this.w;
		...
		if (initialAnim && !w.globals.resized && !w.globals.dataChanged) anim.animatePathsGradually(__spreadProps(__spreadValues({}, defaultAnimateOpts), { speed: initialSpeed }));
		else if (w.globals.resized || !w.globals.dataChanged) anim.showDelayedElements();
		if (w.globals.dataChanged && dynamicAnim && shouldAnimate) anim.animatePathsGradually(__spreadProps(__spreadValues({}, defaultAnimateOpts), { speed: dataChangeSpeed }));
		return el;
	}
	/**
	* @param {string} style
		...
		for (let i = startingSeriesIndex; i < endingSeriesIndex; i++) {
			...
			for (let j = firstXIndex; j <= lastXIndex && j < this.w.seriesData.series[i].length; j++) {
				...
				if (val !== null && Utils$1.isNumber(val)) {
					...
					if (minY > ((_d = seriesMin[i]) == null ? void 0 : _d[j]) && ((_e = seriesMin[i]) == null ? void 0 : _e[j]) < 0) minY = seriesMin[i][j];
...
var Series = class Series {
	...
	highlightRangeInSeries(e, targetElement) {
		...
		const activeInactive = (action) => {
			for (let i = 0; i < allHeatMapElements.length; i++) {
				const actionFn = allHeatMapElements[i].classList[action];
				if (typeof actionFn === "function") actionFn.call(
					/** @type {any} */
		...
		const removeInactiveClassFromHoveredRange = (range, rangeMax) => {
			for (let i = 0; i < allHeatMapElements.length; i++) {
				...
				if (val >= range.from && (val < range.to || range.to === rangeMax && val === rangeMax)) allHeatMapElements[i].classList.remove(this.legendInactiveClass);
			}
		};
		if (e.type === "mousemove") {
			...
			removeInactiveClassFromHoveredRange(range, ranges.reduce((acc, cur) => Math.max(acc, cur.to), 0));
	...
	*/
	...
	getPreviousPaths() {
		...
		if (!w.globals.axisCharts) w.globals.previousPaths = w.seriesData.series;
	}
	clearPreviousPaths() {
		const w = this.w;
		w.globals.previousPaths = [];
...
var TimeScale = class {
	...
	*/
	calculateTimeScaleTicks(minX, maxX) {
		...
		const params = {
			firstVal,
			currentMillisecond: firstVal.minMillisecond,
			currentSecond: firstVal.minSecond,
			currentMinute: firstVal.minMinute,
			currentHour: firstVal.minHour,
			currentMonthDate: firstVal.minDate,
			currentDate: firstVal.minDate,
			currentMonth: firstVal.minMonth,
			currentYear: firstVal.minYear,
			daysWidthOnXAxis,
			hoursWidthOnXAxis,
			minutesWidthOnXAxis,
			secondsWidthOnXAxis,
		...
		const adjustedMonthInTimeScaleArray = this.timeScaleArray.map((ts) => {
			...
			if (ts.unit === "month") return __spreadProps(__spreadValues({}, defaultReturn), {
				day: 1,
				value: ts.value + 1
			});
			else if (ts.unit === "day" || ts.unit === "hour") return __spreadProps(__spreadValues({}, defaultReturn), { value: ts.value });
			else if (ts.unit === "minute") return __spreadProps(__spreadValues({}, defaultReturn), {
				value: ts.value,
				minute: ts.value
			});
			else if (ts.unit === "second") return __spreadProps(__spreadValues({}, defaultReturn), {
				value: ts.value,
				minute: ts.minute,
				second: ts.second
			});
		...
		return adjustedMonthInTimeScaleArray.filter((ts) => {
			let modulo = 1;
			let ticks = Math.ceil(w.layout.gridWidth / 120);
			const value = ts.value;
			if (w.config.xaxis.tickAmount !== void 0) ticks = w.config.xaxis.tickAmount;
			...
			} else if ((value % modulo === 0 || shouldNotSkipUnit) && !shouldNotPrint) return true;
		});
	}
	/**
	...
	*/
	...
	generateYearScale({ firstVal, currentMonth, currentYear, daysWidthOnXAxis, numberOfYears }) {
		let firstTickValue = firstVal.minYear;
		let firstTickPosition = 0;
		const dt = new DateTime(this.w);
		const unit = "year";
		if (firstVal.minDate > 1 || firstVal.minMonth > 0) {
			const remainingDays = dt.determineRemainingDaysOfYear(firstVal.minYear, firstVal.minMonth, firstVal.minDate);
			firstTickPosition = (dt.determineDaysOfYear(firstVal.minYear) - remainingDays + 1) * daysWidthOnXAxis;
		...
		} else if (firstVal.minDate === 1 && firstVal.minMonth === 0) this.timeScaleArray.push({
			position: firstTickPosition,
			value: firstTickValue,
			unit,
			year: currentYear,
	...
	generateMonthScale({ firstVal, currentMonthDate, currentMonth, currentYear, daysWidthOnXAxis, numberOfMonths }) {
		...
		if (firstVal.minDate > 1) {
			firstTickPosition = (dt.determineDaysOfMonths(currentMonth + 1, firstVal.minYear) - currentMonthDate + 1) * daysWidthOnXAxis;
	...
	generateDayScale({ firstVal, currentMonth, currentYear, hoursWidthOnXAxis, numberOfDays }) {
		const dt = new DateTime(this.w);
		let unit = "day";
		let firstTickValue = firstVal.minDate + 1;
		let date = firstTickValue;
		const changeMonth = (dateVal, month2, year) => {
			if (dateVal > dt.determineDaysOfMonths(month2 + 1, year)) {
				month2 = month2 + 1;
				date = 1;
				unit = "month";
				val = month2;
	...
	generateHourScale({ firstVal, currentDate, currentMonth, currentYear, minutesWidthOnXAxis, numberOfHours }) {
		const dt = new DateTime(this.w);
		const yrCounter = 0;
		let unit = "hour";
		const changeDate = (dateVal, month2) => {
			if (dateVal > dt.determineDaysOfMonths(month2 + 1, currentYear)) {
				date = 1;
				month2 = month2 + 1;
			}
			return {
		...
		let month = changeDate(date, currentMonth).month;
		month = changeMonth(date, month);
		if (unit === "day") firstTickValue = date;
		this.timeScaleArray.push({
			position: firstTickPosition,
			value: firstTickValue,
			unit,
		...
		for (let i = 0; i < numberOfHours; i++) {
			...
			if (hour >= 24) {
				...
				date += 1;
				unit = "day";
				month = changeDate(date, month).month;
				month = changeMonth(date, month);
			}
			const year = this._getYear(currentYear, month, yrCounter);
	...
	generateMinuteScale({ currentMillisecond, currentSecond, currentMinute, currentHour, currentDate, currentMonth, currentYear, minutesWidthOnXAxis, secondsWidthOnXAxis, numberOfMinutes }) {
		const dt = new DateTime(this.w);
		const yrCounter = 0;
		const unit = "minute";
		let firstTickPosition = (60 - currentSecond - currentMillisecond / 1e3) * secondsWidthOnXAxis;
		...
		for (let i = 0; i < numberOfMinutes; i++) {
			if (minute >= 60) {
				...
				if (hour === 24) {
					hour = 0;
					date += 1;
					const monthDays = dt.determineDaysOfMonths(month + 1, this._getYear(year, month, yrCounter));
					if (date > monthDays) {
						date = 1;
						month += 1;
					}
	...
	generateSecondScale({ currentMillisecond, currentSecond, currentMinute, currentHour, currentDate, currentMonth, currentYear, secondsWidthOnXAxis, numberOfSeconds }) {
		const yrCounter = 0;
		const unit = "second";
		let firstTickPosition = (1e3 - currentMillisecond) / 1e3 * secondsWidthOnXAxis;
	...
	*/
	createRawDateString(ts, value) {
		let raw = ts.year;
		if (ts.month === 0) ts.month = 1;
		raw += "-" + ("0" + ts.month.toString()).slice(-2);
		if (ts.unit === "day") raw += "-" + ("0" + value).slice(-2);
		else raw += "-" + ("0" + (ts.day ? ts.day : "1")).slice(-2);
		if (ts.unit === "hour") raw += "T" + ("0" + value).slice(-2);
	...
	*/
	removeOverlappingTS(arr) {
		...
		if (arr.length > 0 && arr[0].value && arr.every((lb) => lb.value.length === arr[0].value.length)) {
			equalLabelLengthFlag = true;
			constantLabelWidth = graphics.getTextRects(arr[0].value, this.w.config.xaxis.labels.style.fontSize).width;
		}
...
var Core = class {
	...
	*/
	...
	setupElements() {
		...
		const SVG2 = Environment.isBrowser() ? window.SVG : 
		/** @type {any} */
global.SVG;
	...
	plotChartType(ser, xyRatios) {
		...
		this.w.seriesData.series.forEach((serie, st) => {
			...
			if (seriesTypes[seriesType]) {
				...
				} else seriesTypes[seriesType].series.push(serie);
	...
	*/
	...
	getAccessibleChartLabel() {
		...
		const cnf = w.config;
		let label = "";
		if (cnf.chart.accessibility && cnf.chart.accessibility.description) label = cnf.chart.accessibility.description;
		else if (cnf.title.text) {
			const chartType = cnf.chart.type;
			label = `${cnf.title.text}. ${chartType} chart`;
			if (cnf.subtitle.text) label += `. ${cnf.subtitle.text}`;
		} else label = `${cnf.chart.type} chart with ${w.seriesData.series.length || (cnf.series ? cnf.series.length : 0)} data series`;
		return label;
	}
};
var Data = class Data {
	...
	*/
	...
	getFirstDataPoint() {
		...
		this.activeSeriesIndex = new Series(this.w).getActiveConfigSeriesIndex();
	...
	*/
	parseDataAxisCharts(ser) {
		...
		for (let i = 0; i < ser.length; i++) {
			...
			if ((dr == null ? void 0 : dr.enabled) && this.isMultiFormat() && ser[i].data.length > ((_a = dr.threshold) != null ? _a : 500)) ser[i] = __spreadProps(__spreadValues({}, ser[i]), { data: Data.lttbDownsample(ser[i].data, (_b = dr.targetPoints) != null ? _b : 250) });
	...
	*/
	...
	printLabels({ i, j, values, ttItems, shared, e }) {
		...
		for (let t = 0, inverset = w.seriesData.series.length - 1; t < w.seriesData.series.length; t++, inverset--) {
			...
			if (w.config.chart.type === "treemap") seriesName = f.yLbTitleFormatter(String(
				/** @type {any} */
			...
			if (w.globals.axisCharts) {
				const getValBySeriesIndex = (index) => {
					var _a2, _b, _c, _d;
					if (w.axisFlags.isRangeData) return f.yLbFormatter((_b = (_a2 = w.rangeData.seriesRangeStart) == null ? void 0 : _a2[index]) == null ? void 0 : _b[j], {
				...
				if (shared) {
					...
					if (hasGoalValues(tIndex)) goalVals = w.seriesData.seriesGoals[tIndex][j].map((goal) => {
						return {
							attrs: goal,
					...
					if (targetFill) if (targetFill.indexOf("url") !== -1) {
						if (targetFill.indexOf("Pattern") !== -1) pColor = w.dom.baseEl.querySelector(targetFill.substr(4).slice(0, -1)).childNodes[0].getAttribute("stroke");
					} else pColor = targetFill;
					val = getValBySeriesIndex(i);
					if (hasGoalValues(i) && Array.isArray(w.seriesData.seriesGoals[i][j])) goalVals = w.seriesData.seriesGoals[i][j].map((goal) => {
						return {
							attrs: goal,
							val: f.yLbFormatter(goal.value, {
								seriesIndex: i,
...
var Position = class {
	...
	*/
	moveYAxisTooltip(index) {
		...
		if (ttCtx.yaxisTTEls) {
			...
			if (w.config.yaxis[index].opposite) cx = cx - yAxisTTRect.width;
	...
	*/
	moveTooltip(cx, cy, markerSize = null) {
		...
		if (x > w.layout.gridWidth / 2) x = x - tooltipRect.ttWidth - pointSize - 10;
		if (x > w.layout.gridWidth - tooltipRect.ttWidth - 10) x = w.layout.gridWidth - tooltipRect.ttWidth;
		if (x < -20) x = -20;
		if (w.config.tooltip.followCursor) {
			...
			if (x > w.layout.gridWidth / 2) x = x - ttCtx.tooltipRect.ttWidth;
			y = ttCtx.e.clientY + w.layout.translateY - seriesBound.top;
			if (y > w.layout.gridHeight / 2) y = y - ttCtx.tooltipRect.ttHeight;
		} else if (!w.globals.isBarHorizontal) {
			if (tooltipRect.ttHeight / 2 + y > w.layout.gridHeight) y = w.layout.gridHeight - tooltipRect.ttHeight + w.layout.translateY;
		}
		if (!isNaN(x)) {
	...
	*/
	moveDynamicPointOnHover(j, capturedSeries) {
		...
		if (serType && (serType === "column" || serType === "candlestick" || serType === "boxPlot")) return;
	...
	*/
	moveStickyTooltipOverBars(j, capturedSeries) {
		...
		let jBar = w.dom.baseEl.querySelector(`.apexcharts-bar-series .apexcharts-series[rel='${i}'] path[j='${j}'], .apexcharts-candlestick-series .apexcharts-series[rel='${i}'] path[j='${j}'], .apexcharts-boxPlot-series .apexcharts-series[rel='${i}'] path[j='${j}'], .apexcharts-rangebar-series .apexcharts-series[rel='${i}'] path[j='${j}']`);
		...
		if (!ttCtx.fixedTooltip) this.moveTooltip(bcx, bcy || w.layout.gridHeight);
	}
};
var Marker = class {
	/**
	...
	*/
	drawTooltip(xyRatios) {
		...
		if (w.globals.axisCharts) {
			...
			this.xAxisTicksPositions = new XAxis(this.w, this.ctx, void 0).getXAxisTicksPositions();
		}
	...
	*/
	...
	addSVGEvents() {
		...
		if (w.globals.axisCharts) {
			...
			else if (commonBar) points = w.dom.baseEl.querySelectorAll(".apexcharts-series .apexcharts-bar-area, .apexcharts-series .apexcharts-candlestick-area, .apexcharts-series .apexcharts-boxPlot-area, .apexcharts-series .apexcharts-rangebar-area");
...
function arcToBezier(pos, val) {
	...
	mat = new Matrix(0, 0, 0, 0, 0, 0).rotate(xAxisRotation).scale(1 / rx, 1 / ry).rotate(-xAxisRotation);
	A = A.transform(mat);
	B = B.transform(mat);
	k = [B.x - A.x, B.y - A.y];
	...
	mat = new Matrix(0, 0, 0, 0, 0, 0).rotate(xAxisRotation).scale(rx, ry).rotate(-xAxisRotation);
...
function installSelectable(ElementClass) {
	ElementClass.prototype.select = function(opts) {
		...
		handleNames.forEach((name2, index) => {
			...
			handles[name2] = {
				group: subGroup,
				handle: createHandle(subGroup, [0, 0], index, [], name2)
			};
		});
...
var ApexCharts = class ApexCharts {
	...
	*/
	destroy() {
		...
		if (chartID) Apex._chartInstances.forEach((c, i) => {
	...
	*/
	static initOnLoad() {
		...
		for (let i = 0; i < els.length; i++) {
			const el = els[i];
			new ApexCharts(el, JSON.parse((_a = els[i].getAttribute("data-options")) != null ? _a : "")).render();
		}
	}
	/**
...
var Exports = class {
	...
	*/
	getSvgString(_scale) {
      ...
      `;
			if (shouldIncludeLegendStyles) exportStyles += apexchartsLegendCSS;
			let svgString = `
      ...
      `;
			...
			if (scale !== 1) this.scaleSvgNode(svgNode, scale);
	...
	*/
	convertImagesToBase64(svgNode) {
		...
		const promises = Array.from(images).map((img) => {
			...
			if (href && !href.startsWith("data:")) return this.getBase64FromUrl(href).then((base64) => {
	...
	*/
		...
		return new Promise((resolve) => {
			...
			this.getSvgString(scale).then((svgData) => {
				...
				img.onload = () => {
					...
					if (edgeCanvas.msToBlob) resolve({ blob: edgeCanvas.msToBlob() });
					else resolve({ imgURI: canvas.toDataURL("image/png") });
				};
				img.src = svgUrl;
			});
		});
	...
	exportToPng() {
		...
		this.dataURI(option).then(({ imgURI, blob }) => {
			if (blob) navigator.msSaveOrOpenBlob(blob, this.w.globals.chartID + ".png");
			else this.triggerDownload(imgURI, this.w.config.chart.toolbar.export.png.filename, ".png");
		});
	}
	/** @param {{ series?: any, fileName?: any, columnDelimiter?: string, lineDelimiter?: string }} opts */
	exportToCSV({ series, fileName, columnDelimiter = ",", lineDelimiter = "\n" }) {
		const w = this.w;
		...
		const getFormattedCategory = (cat) => {
			if (typeof w.config.chart.toolbar.export.csv.categoryFormatter === "function") return w.config.chart.toolbar.export.csv.categoryFormatter(cat);
			if (w.config.xaxis.type === "datetime" && String(cat).length >= 10) return new Date(cat).toDateString();
		...
		const getCat = (i) => {
			...
			if (!w.globals.axisCharts) cat = w.config.labels[i];
				...
				if (w.config.xaxis.type === "category" || w.config.xaxis.convertedCatToNumeric) if (w.globals.isBarHorizontal) {
					const lbFormatter = w.formatters.yLabelFormatters[0];
					const activeSeries = new Series(this.ctx.w).getActiveConfigSeriesIndex();
					cat = lbFormatter(w.labelData.labels[i], {
						seriesIndex: activeSeries,
						dataPointIndex: i,
						w
					});
				} else cat = axesUtils.getLabel(w.labelData.labels, w.labelData.timescaleLabels, 0, i).text;
				if (w.config.xaxis.type === "datetime") {
		...
		const handleUnequalXValues = () => {
			...
			series.forEach((s, sI) => {
				s?.data.forEach((dataItem) => {
					...
					if (!data[cat]) data[cat] = Array(series.length).fill("");
		...
		if (w.config.chart.type === "boxPlot") {
			...
			columns.push("minimum");
			columns.push("maximum");
		} else series.map((s, sI) => {
			const sname = (s.name ? s.name : `series-${sI}`) + "";
			if (w.globals.axisCharts) columns.push(sname.split(columnDelimiter).join("") ? sname.split(columnDelimiter).join("") : `series-${sI}`);
		});
		if (!w.globals.axisCharts) {
			columns.push(w.config.chart.toolbar.export.csv.headerValue);
			rows.push(columns.join(columnDelimiter));
		}
		if (!w.globals.allSeriesHasEqualX && w.globals.axisCharts && !w.config.xaxis.categories.length && !w.config.labels.length) handleUnequalXValues();
		else series.map((s, sI) => {
...
var Helpers$3 = class Helpers2 {
	...
	*/
	...
	getLegendStyles() {
		...
		if (nonce) stylesheet.setAttribute("nonce", nonce);
		const rule = document.createTextNode(apexchartsLegendCSS);
		stylesheet.appendChild(rule);
		return stylesheet;
	}
	getLegendDimensions() {
		const currLegendsWrap = this.w.dom.baseEl.querySelector(".apexcharts-legend");
	...
	appendToForeignObject() {
		...
		if (this.w.config.chart.injectStyleSheet !== false && legendStyles) (_a = this.w.dom.elLegendForeign) == null || _a.appendChild(legendStyles);
	}
	/**
	...
	*/
	toggleDataSeries(seriesCnt, isHidden) {
		...
		if (w.globals.axisCharts || w.config.chart.type === "radialBar") {
			...
			if (w.globals.axisCharts) {
				seriesEl = w.dom.baseEl.querySelector(`.apexcharts-series[data\\:realIndex='${seriesCnt}']`);
				if (!seriesEl) return;
				realIndex = parseInt((_a = seriesEl.getAttribute("data:realIndex")) != null ? _a : "", 10);
			} else {
				seriesEl = w.dom.baseEl.querySelector(`.apexcharts-series[rel='${seriesCnt + 1}']`);
				if (!seriesEl) return;
			...
			if (w.config.chart.accessibility.enabled) {
				const legendItem = w.dom.baseEl.querySelector(`.apexcharts-legend-series[rel="${seriesCnt + 1}"]`);
				if (legendItem) {
			...
			const seriesEl = w.dom.Paper.findOne(` .apexcharts-series[rel='${seriesCnt + 1}'] path`);
			const type = w.config.chart.type;
			if (type === "pie" || type === "polarArea" || type === "donut") {
				const dataLabels = w.config.plotOptions.pie.donut.labels;
				new Graphics(this.w).pathMouseDown(seriesEl, null);
	...
	getSeriesAfterCollapsing({ realIndex }) {
		...
		if (gl.axisCharts) {
			...
			} else if (gl.collapsedSeriesIndices.indexOf(realIndex) < 0) {
			...
			gl.collapsedSeries.push({
				...
				type: (_a = w.config.series[realIndex].type) != null ? _a : "line"
			});
			gl.collapsedSeriesIndices.push(realIndex);
		}
	...
	hideSeries({ seriesEl, realIndex }) {
		const w = this.w;
		const series = this.getSeriesAfterCollapsing({ realIndex });
		const seriesChildren = seriesEl.childNodes;
		for (let sc = 0; sc < seriesChildren.length; sc++) if (seriesChildren[sc].classList.contains("apexcharts-series-markers-wrap")) if (seriesChildren[sc].classList.contains("apexcharts-hide")) seriesChildren[sc].classList.remove("apexcharts-hide");
		else seriesChildren[sc].classList.add("apexcharts-hide");
		this.lgCtx.updateSeries(series, w.config.chart.animations.dynamicAnimation.enabled);
	}
	/**
	...
	*/
	riseCollapsedSeries(collapsedSeries, seriesIndices, realIndex) {
		...
		if (collapsedSeries.length > 0) {
			for (let c = 0; c < collapsedSeries.length; c++) if (collapsedSeries[c].index === realIndex) {
				if (w.globals.axisCharts) series[realIndex].data = collapsedSeries[c].data.slice();
				else series[realIndex] = collapsedSeries[c].data;
				if (typeof series[realIndex] !== "number") series[realIndex].hidden = false;
	...
	*/
	_getSeriesBasedOnCollapsedState(series) {
		const w = this.w;
		let collapsed = 0;
		if (w.globals.axisCharts) series.forEach((s, sI) => {
	...
	*/
	...
	init() {
		...
		if ((showLegendAlways || !gl.axisCharts) && cnf.legend.show) {
			const elLegendWrap = w.dom.elLegendWrap;
			while (elLegendWrap.firstChild) elLegendWrap.removeChild(elLegendWrap.firstChild);
			this.drawLegends();
	...
	createLegendMarker({ i, fillcolor }) {
		...
		if (w.config.legend.markers.customHTML) {
			...
			const markerConfig = new Markers(this.ctx.w, this.ctx).getMarkerConfig({
				cssClass: `apexcharts-legend-marker apexcharts-marker apexcharts-marker-${shape}`,
				seriesIndex: i,
				strokeWidth: mBorderWidth,
				size: mSize
			});
			const SVGMarker = (Environment.isBrowser() ? window.SVG : 
			/** @type {any} */
global.SVG)().addTo(elMarker).size("100%", "100%");
			...
			w.dom.Paper.find(".apexcharts-legend-marker.apexcharts-marker").forEach((shapeEl) => {
				if (shapeEl.node.classList.contains("apexcharts-marker-triangle")) shapeEl.node.style.transform = "translate(50%, 45%)";
				else shapeEl.node.style.transform = "translate(50%, 50%)";
			});
	...
	drawLegends() {
		...
		} else if (this.isBarsDistributed) legendNames = w.labelData.labels.slice();
		...
		const legendGroups = [];
		if (w.labelData.seriesGroups.length > 1 && w.config.legend.clusterGroupedSeries) w.labelData.seriesGroups.forEach((_, gi) => {
			...
			if (w.config.legend.clusterGroupedSeriesOrientation === "horizontal") elLegendWrap.classList.add("apexcharts-legend-group-horizontal");
			else legendGroups[gi].classList.add("apexcharts-legend-group-vertical");
		});
		for (let i = isLegendInversed ? legendNames.length - 1 : 0; isLegendInversed ? i >= 0 : i <= legendNames.length - 1; isLegendInversed ? i-- : i++) {
			const text = legendFormatter(legendNames[i], {
				seriesIndex: i,
				w
			});
			...
			if (w.globals.collapsedSeries.length > 0) {
				for (let c = 0; c < w.globals.collapsedSeries.length; c++) if (w.globals.collapsedSeries[c].index === i) collapsedSeries = true;
			}
			if (w.globals.ancillaryCollapsedSeriesIndices.length > 0) {
				for (let c = 0; c < w.globals.ancillaryCollapsedSeriesIndices.length; c++) if (w.globals.ancillaryCollapsedSeriesIndices[c] === i) ancillaryCollapsedSeries = true;
			}
			const elMarker = this.createLegendMarker({
				i,
				fillcolor
			});
			...
			if (!w.config.legend.showForZeroSeries) {
				if (coreUtils.getSeriesTotalByIndex(i) === 0 && coreUtils.seriesHaveSameValues(i) && !coreUtils.isSeriesNull(i) && w.globals.collapsedSeriesIndices.indexOf(i) === -1 && w.globals.ancillaryCollapsedSeriesIndices.indexOf(i) === -1) elLegend.classList.add("apexcharts-hidden-zero-series");
			}
			...
			if (legendGroups.length) w.labelData.seriesGroups.forEach((group, gi) => {
				var _a2, _b;
				if (group.includes(
					/** @type {Record<string,any>} */
			...
			elLegendWrap.classList.add(`apexcharts-align-${w.config.legend.horizontalAlign}`);
			elLegendWrap.classList.add("apx-legend-position-" + w.config.legend.position);
		...
		if (w.config.chart.accessibility.enabled && w.config.chart.accessibility.keyboard.enabled) w.dom.elWrap.addEventListener("keydown", me.onLegendKeyDown.bind(me), true);
	}
	/**
	...
	*/
	setLegendWrapXY(offsetX, offsetY) {
		...
		let x = 0;
		let y = 0;
		if (w.config.legend.position === "bottom") y = w.globals.svgHeight - Math.min(legendHeight, w.globals.svgHeight / 2) - 5;
		...
		["width", "height"].forEach((hw) => {
			if (elLegendWrap && elLegendWrap.style[hw]) elLegendWrap.style[hw] = parseInt(String(w.config.legend[hw]), 10) + "px";
		});
	}
	legendAlignHorizontal() {
		...
		const offsetX = 20;
		let offsetY = 0;
		if (w.config.legend.position === "top") offsetY = titleRect.height + subtitleRect.height + w.config.title.margin + w.config.subtitle.margin - 10;
	...
	*/
	onLegendHovered(e) {
		...
		if (w.config.chart.type !== "heatmap" && !this.isBarsDistributed) {
			if (!target.classList.contains("apexcharts-inactive-legend") && hoverOverLegend) new Series(this.ctx.w).toggleSeriesOnHover(e, target);
		} else if (hoverOverLegend) {
			...
			new Series(this.ctx.w).highlightRangeInSeries(e, target);
		}
	}
	/**
	...
	*/
	onLegendKeyDown(e) {
		...
		const w = this.w;
		const target = e.target;
		if (!(target.classList.contains("apexcharts-legend-series") || target.classList.contains("apexcharts-legend-text") || target.classList.contains("apexcharts-legend-marker"))) return;
		if (e.key === "Enter" || e.key === " ") {
			e.preventDefault();
			const rel = target.getAttribute("rel");
			me.onLegendClick(e);
			if (rel !== null && w.config.legend.onItemClick.toggleDataSeries) requestAnimationFrame(() => {
				const restored = w.dom.baseEl.querySelector(`.apexcharts-legend-series[rel="${rel}"]`);
	...
	*/
	onLegendClick(e) {
		...
		if (target.classList.contains("apexcharts-legend-series") || target.classList.contains("apexcharts-legend-text") || target.classList.contains("apexcharts-legend-marker")) {
			...
			if (w.config.chart.type !== "treemap" && w.config.chart.type !== "heatmap" && !this.isBarsDistributed && w.config.legend.onItemClick.toggleDataSeries) this.legendHelpers.toggleDataSeries(seriesCnt, isHidden);
		}
	}
};
...
var Toolbar = class {
	...
	*/
	...
	createToolbar() {
		...
		if (Array.isArray(this.t.customIcons)) for (let i = 0; i < this.t.customIcons.length; i++) this.elCustomIcons.push(createDiv());
		const toolbarControls = [];
		const appendZoomControl = (type, el, ico) => {
			const tool = type.toLowerCase();
			if (this.t[tool] && w.config.chart.zoom.enabled) toolbarControls.push({
				el,
		...
		const zoomSelectionCtrls = (z) => {
			if (this.t[z] && w.config.chart[z].enabled) toolbarControls.push({
		...
		if (this.t.pan && w.config.chart.zoom.enabled) toolbarControls.push({
			el: this.elPan,
		...
		if (this.t.download) toolbarControls.push({
			el: this.elMenuIcon,
			icon: typeof this.t.download === "string" ? this.t.download : icoMenu,
			title: this.localeValues.menu,
			class: "apexcharts-menu-icon"
		});
		for (let i = 0; i < this.elCustomIcons.length; i++) toolbarControls.push({
		...
		if (this.elZoom.parentNode) this.elZoom.setAttribute("aria-pressed", String(!!w.interact.zoomEnabled));
		if (this.elSelection.parentNode) this.elSelection.setAttribute("aria-pressed", String(!!w.interact.selectionEnabled));
		if (this.elPan.parentNode) this.elPan.setAttribute("aria-pressed", String(!!w.interact.panEnabled));
		...
		if (w.interact.zoomEnabled) this.elZoom.classList.add(this.selectedClass);
		else if (w.interact.panEnabled) this.elPan.classList.add(this.selectedClass);
		else if (w.interact.selectionEnabled) this.elSelection.classList.add(this.selectedClass);
	...
	*/
	...
	addToolbarEventListeners() {
		var _a, _b, _c, _d, _e, _f, _g, _h;
		(_a = this.elZoomReset) == null || _a.addEventListener("click", this.handleZoomReset.bind(this));
		(_b = this.elSelection) == null || _b.addEventListener("click", this.toggleZoomSelection.bind(this, "selection"));
		(_c = this.elZoom) == null || _c.addEventListener("click", this.toggleZoomSelection.bind(this, "zoom"));
		(_d = this.elZoomIn) == null || _d.addEventListener("click", this.handleZoomIn.bind(this));
		(_e = this.elZoomOut) == null || _e.addEventListener("click", this.handleZoomOut.bind(this));
		(_f = this.elPan) == null || _f.addEventListener("click", this.togglePanning.bind(this));
		(_g = this.elMenuIcon) == null || _g.addEventListener("click", this.toggleMenu.bind(this));
		this.elMenuItems.forEach((m) => {
			if (m.classList.contains("exportSVG")) m.addEventListener("click", this.handleDownload.bind(this, "svg"));
			else if (m.classList.contains("exportPNG")) m.addEventListener("click", this.handleDownload.bind(this, "png"));
			else if (m.classList.contains("exportCSV")) m.addEventListener("click", this.handleDownload.bind(this, "csv"));
		});
		for (let i = 0; i < this.t.customIcons.length; i++) this.elCustomIcons[i].addEventListener("click", this.t.customIcons[i].click.bind(this, this.ctx, this.ctx.w));
		[
			this.elZoomReset,
			this.elSelection,
			this.elZoom,
		...
		(_h = this.elMenuIcon) == null || _h.addEventListener("keydown", (e) => {
			var _a2;
			if (e.key === "ArrowDown" || e.key === "ArrowUp") {
				e.preventDefault();
				if (!((_a2 = this.elMenu) == null ? void 0 : _a2.classList.contains("apexcharts-menu-open"))) this.toggleMenu();
				window.setTimeout(() => {
		...
		this.elMenuItems.forEach((m, idx) => {
			m.addEventListener("keydown", (e) => {
				...
				if (e.key === "ArrowDown") {
					e.preventDefault();
					(this.elMenuItems[idx + 1] || this.elMenuItems[0]).focus();
				} else if (e.key === "ArrowUp") {
					e.preventDefault();
					(this.elMenuItems[idx - 1] || this.elMenuItems[this.elMenuItems.length - 1]).focus();
				} else if (e.key === "Escape" || e.key === "Tab") {
					this._closeMenu();
					(_a2 = this.elMenuIcon) == null || _a2.focus();
					if (e.key === "Tab");
	...
	*/
	toggleZoomSelection(type) {
		this.ctx.getSyncedCharts().forEach((ch) => {
	...
	getToolbarIconsReference() {
		const w = this.w;
		if (!this.elZoom) this.elZoom = w.dom.baseEl.querySelector(".apexcharts-zoom-icon");
		if (!this.elPan) this.elPan = w.dom.baseEl.querySelector(".apexcharts-pan-icon");
		if (!this.elSelection) this.elSelection = w.dom.baseEl.querySelector(".apexcharts-selection-icon");
	}
	/**
	* @param {string} type
	*/
	...
	togglePanning() {
		this.ctx.getSyncedCharts().forEach((ch) => {
			...
			ch.ctx.toolbar.elPan.setAttribute("aria-pressed", String(ch.w.interact.panEnabled));
		});
	}
	toggleOtherControls() {
		const w = this.w;
		...
		[
			...
			if (el) el.classList.remove(this.selectedClass);
		});
	}
	handleZoomIn() {
		const w = this.w;
		...
		if (!w.interact.disableZoomIn) this.zoomUpdateOptions(newMinXMaxX.minX, newMinXMaxX.maxX);
	}
	handleZoomOut() {
		const w = this.w;
		if (w.axisFlags.isRangeBar) {
		...
		if (w.config.xaxis.type === "datetime" && new Date(this.minX).getUTCFullYear() < 1e3) return;
		const centerX = (this.minX + this.maxX) / 2;
		const newMinX = this.minX - (centerX - this.minX);
		const newMaxX = this.maxX - (centerX - this.maxX);
		const newMinXMaxX = this._getNewMinXMaxX(newMinX, newMaxX);
		if (!w.interact.disableZoomOut) this.zoomUpdateOptions(newMinXMaxX.minX, newMinXMaxX.maxX);
	}
	/**
	...
	*/
	zoomUpdateOptions(newMinX, newMaxX) {
		...
		const options2 = { xaxis };
		if (!w.globals.initialConfig) return;
		const yaxis = Utils$1.clone(w.globals.initialConfig.yaxis);
		if (!w.config.chart.group) options2.yaxis = yaxis;
	...
	*/
	getBeforeZoomRange(xaxis, yaxis) {
		let newRange = null;
		if (typeof this.ev.beforeZoom === "function") newRange = this.ev.beforeZoom(this, {
			xaxis,
			yaxis
		});
		return newRange;
	...
	toggleMenu() {
		window.setTimeout(() => {
			var _a, _b, _c;
			if ((_a = this.elMenu) == null ? void 0 : _a.classList.contains("apexcharts-menu-open")) this._closeMenu();
			else {
				(_b = this.elMenu) == null || _b.classList.add("apexcharts-menu-open");
				(_c = this.elMenuIcon) == null || _c.setAttribute("aria-expanded", "true");
			}
		}, 0);
	}
	_closeMenu() {
		var _a, _b;
		(_a = this.elMenu) == null || _a.classList.remove("apexcharts-menu-open");
		(_b = this.elMenuIcon) == null || _b.setAttribute("aria-expanded", "false");
	}
	/**
	* @param {string} type
	*/
	...
	handleZoomReset() {
		this.ctx.getSyncedCharts().forEach((ch) => {
			...
			if (typeof w.config.chart.events.zoomed === "function") ch.ctx.toolbar.zoomCallback({
				min: w.config.xaxis.min,
				max: w.config.xaxis.max
			});
			const series = ch.ctx.series.emptyCollapsedSeries(Utils$1.clone(w.globals.initialSeries));
...
var ZoomPanSelection = class extends Toolbar {
	...
	*/
	...
	init({ xyRatios }) {
		...
		if (w.config.chart.selection.type === "x") this.slDraggableRect = this.selectionRect.draggable({
			minX: 0,
			minY: 0,
			maxX: w.layout.gridWidth,
			maxY: w.layout.gridHeight
		}).on("dragmove.namespace", this.selectionDragging.bind(this, "dragging"));
		else if (w.config.chart.selection.type === "y") this.slDraggableRect = this.selectionRect.draggable({
			minX: 0,
			maxX: w.layout.gridWidth
		}).on("dragmove.namespace", this.selectionDragging.bind(this, "dragging"));
		else this.slDraggableRect = this.selectionRect.draggable().on("dragmove.namespace", this.selectionDragging.bind(this, "dragging"));
		this.preselectedSelection();
		this.hoverArea = w.dom.baseEl.querySelector(`${w.globals.chartClass} .apexcharts-svg`);
		if (!this.hoverArea) return;
		this.hoverArea.classList.add("apexcharts-zoomable");
		this.eventList.forEach((event) => {
			var _a;
			(_a = this.hoverArea) == null || _a.addEventListener(event, me.svgMouseEvents.bind(me, xyRatios), {
				capture: false,
				passive: true
			});
		});
		if (w.config.chart.zoom.enabled && w.config.chart.zoom.allowMouseWheelZoom) this.hoverArea.addEventListener("wheel", me.mouseWheelEvent.bind(me), {
			capture: false,
			passive: false
		});
	}
	...
	*/
	svgMouseEvents(xyRatios, e) {
		...
		let pc;
		if (e.target.parentNode && e.target.parentNode !== null) pc = e.target.parentNode.classList;
		if (tc.contains("apexcharts-legend-marker") || tc.contains("apexcharts-legend-text") || pc && pc.contains("apexcharts-toolbar")) return;
		...
		if (e.type === "mousemove" && e.which === 1 || e.type === "touchmove") {
			...
			if (w.interact.panEnabled) {
				...
				if (this.w.interact.mousedown) this.panDragging({
					context: this,
					zoomtype,
					xyRatios
				});
			} else if (this.w.interact.mousedown && w.interact.zoomEnabled || this.w.interact.mousedown && w.interact.selectionEnabled) this.selection = this.selectionDrawing({
				context: this,
				zoomtype
			});
		}
		if (e.type === "mouseup" || e.type === "touchend" || e.type === "mouseleave") this.handleMouseUp({ zoomtype });
	...
	handleMouseUp({ zoomtype, isResized }) {
		...
		if (gridRectDim && (this.w.interact.mousedown || isResized)) {
			...
			if (w.interact.zoomEnabled || w.interact.selectionEnabled) this.selectionDrawn({
				context: this,
				zoomtype
			});
		}
		if (w.interact.zoomEnabled) this.hideSelectionRect(this.selectionRect);
		this.dragged = false;
	...
	*/
	executeMouseWheelZoom(e) {
		...
		if (!w.axisFlags.isRangeBar) {
			...
			const minRange = (w.globals.initialMaxX - w.globals.initialMinX) * .01;
		...
		const newMinXMaxX = this._getNewMinXMaxX(newMinX, newMaxX);
		if (!isNaN(newMinXMaxX.minX) && !isNaN(newMinXMaxX.maxX)) this.zoomUpdateOptions(newMinXMaxX.minX, newMinXMaxX.maxX);
	}
	...
	preselectedSelection() {
		...
		if (!w.interact.zoomEnabled) {
			if (typeof w.interact.selection !== "undefined" && w.interact.selection !== null) this.drawSelectionRect(__spreadProps(__spreadValues({}, w.interact.selection), {
				translateX: w.layout.translateX,
				translateY: w.layout.translateY
			}));
			else if (w.config.chart.selection.xaxis.min !== void 0 && w.config.chart.selection.xaxis.max !== void 0) {
				...
				this.makeSelectionRectDraggable();
				if (typeof w.config.chart.events.selection === "function") w.config.chart.events.selection(this.ctx, {
					xaxis: {
	...
	drawSelectionRect({ x, y, width, height, translateX = 0, translateY = 0 }) {
		...
		if (this.dragged || w.interact.selection !== null) {
			const scalingAttrs = { transform: "translate(" + translateX + ", " + translateY + ")" };
	...
	*/
	...
	selectionDrawing({ context, zoomtype }) {
		...
		if (Math.abs(selectionWidth + startX) > w.layout.gridWidth) selectionWidth = w.layout.gridWidth - startX;
		else if (left < 0) selectionWidth = startX;
		if (startX > left) {
			inversedX = true;
		...
		if (zoomtype === "x") selectionRect = {
			x: inversedX ? startX - selectionWidth : startX,
			y: 0,
			width: selectionWidth,
			height: w.layout.gridHeight
		};
		else if (zoomtype === "y") selectionRect = {
			x: 0,
	...
	*/
	selectionDragging(type, e) {
		...
		if (box.x2 > constraints.x2) x = constraints.x2 - box.w;
		if (box.y2 > constraints.y2) y = constraints.y2 - box.h;
		handler.move(x, y);
		const xyRatios = this.xyRatios;
		const selRect = this.selectionRect;
		let timerInterval = 0;
		if (type === "resizing") timerInterval = 30;
		...
		if (typeof w.config.chart.events.selection === "function" && w.interact.selectionEnabled) {
			...
			this.w.globals.selectionResizeTimer = window.setTimeout(() => {
				...
				if (w.config.chart.brush.enabled && w.config.chart.events.brushScrolled !== void 0) w.config.chart.events.brushScrolled(this.ctx, xyAxis);
			}, timerInterval);
		}
	}
	...
	selectionDrawn({ context, zoomtype }) {
		...
		if (me.dragged && (me.dragX > 10 || me.dragY > 10) && xLowestValue !== xHighestValue) {
			if (w.interact.zoomEnabled) {
				...
				if (zoomtype === "xy" || zoomtype === "x") xaxis = {
					min: xLowestValue,
					max: xHighestValue
				};
				if (zoomtype === "xy" || zoomtype === "y") yaxis.forEach((yaxe, index) => {
				...
				if (zoomtype === "xy" || zoomtype === "y") {
					const yaxisCopy = Utils$1.clone(w.config.yaxis);
					yaxis = yaxisCopy;
				...
				if (typeof w.config.chart.events.selection === "function") w.config.chart.events.selection(me.ctx, {
					xaxis,
					yaxis
				});
			}
...
var Helpers$2 = class Helpers3 {
	...
	*/
	setOrientations(anno, annoIndex = null) {
		...
		if (anno.label.orientation === "vertical") {
			...
			const xAnno = w.dom.baseEl.querySelector(`.apexcharts-xaxis-annotations .apexcharts-xaxis-annotation-label[rel='${i}']`);
			if (xAnno !== null) {
				const xAnnoCoord = xAnno.getBBox();
				xAnno.setAttribute("x", String(parseFloat((_a = xAnno.getAttribute("x")) != null ? _a : "0") - xAnnoCoord.height + 4));
				const yOffset = anno.label.position === "top" ? xAnnoCoord.width : -xAnnoCoord.width;
				xAnno.setAttribute("y", String(parseFloat((_b = xAnno.getAttribute("y")) != null ? _b : "0") + yOffset));
	...
	*/
	addBackgroundToAnno(annoEl, anno) {
		const w = this.w;
		if (!annoEl || !anno.label.text || !String(anno.label.text).trim()) return null;
		...
		let { left: pleft, right: pright, top: ptop, bottom: pbottom } = anno.label.style.padding;
		if (anno.label.orientation === "vertical") [ptop, pbottom, pleft, pright] = [
			pleft,
			pright,
			ptop,
			pbottom
	...
	annotationsBackground() {
		...
		const add = (anno, i, type) => {
			const annoLabel = w.dom.baseEl.querySelector(`.apexcharts-${type}-annotations .apexcharts-${type}-annotation-label[rel='${i}']`);
			if (annoLabel) {
				const parent = annoLabel.parentNode;
				const elRect = this.addBackgroundToAnno(annoLabel, anno);
				if (elRect) {
					parent?.insertBefore(elRect.node, annoLabel);
					if (anno.label.mouseEnter) elRect.node.addEventListener("mouseenter", anno.label.mouseEnter.bind(this, anno));
					if (anno.label.mouseLeave) elRect.node.addEventListener("mouseleave", anno.label.mouseLeave.bind(this, anno));
					if (anno.label.click) elRect.node.addEventListener("click", anno.label.click.bind(this, anno));
				}
			}
		};
	...
	*/
	getY1Y2(type, anno) {
		...
		if (this.annoCtx.invertAxis) {
			...
			const xLabel = w.dom.baseEl.querySelector(`.apexcharts-yaxis-texts-g text:nth-child(${catIndex + 1})`);
			...
			if (anno.seriesIndex !== void 0 && w.globals.barHeight) yP -= w.globals.barHeight / 2 * (w.seriesData.series.length - 1) - w.globals.barHeight * anno.seriesIndex;
		} else {
			const seriesIndex = w.globals.seriesYAxisMap[anno.yAxisIndex][0];
			const yPos = w.config.yaxis[anno.yAxisIndex].logarithmic ? new CoreUtils(this.w).getLogVal(w.config.yaxis[anno.yAxisIndex].logBase, y, seriesIndex) / w.globals.yLogRatio[seriesIndex] : (y - w.globals.minYArr[seriesIndex]) / (w.globals.yRange[seriesIndex] / w.layout.gridHeight);
			yP = w.layout.gridHeight - Math.min(Math.max(yPos, 0), w.layout.gridHeight);
			clipped = yPos > w.layout.gridHeight || yPos < 0;
			if (anno.marker && (anno.y === void 0 || anno.y === null)) yP = 0;
			if ((_b = w.config.yaxis[anno.yAxisIndex]) == null ? void 0 : _b.reversed) yP = yPos;
		}
		if (typeof y === "string" && y.includes("px")) yP = parseFloat(y);
		return {
			yP,
			clipped
		};
	...
	*/
	getX1X2(type, anno) {
		...
		if ((w.config.xaxis.type === "category" || w.config.xaxis.convertedCatToNumeric) && !this.annoCtx.invertAxis && !w.axisFlags.dataFormatXNumeric) {
			if (!w.config.chart.sparkline.enabled) xP = this.getStringX(x);
		}
		if (typeof x === "string" && x.includes("px")) xP = parseFloat(x);
		if ((x === void 0 || x === null) && anno.marker) xP = w.layout.gridWidth;
		if (anno.seriesIndex !== void 0 && w.globals.barWidth && !this.annoCtx.invertAxis) xP -= w.globals.barWidth / 2 * (w.seriesData.series.length - 1) - w.globals.barWidth * anno.seriesIndex;
		if (typeof xP !== "number") {
			xP = 0;
			clipped = true;
		}
	...
	*/
	getStringX(x) {
		...
		if (w.config.xaxis.convertedCatToNumeric && w.labelData.categoryLabels.length) {
			const strX = String(x);
			x = w.labelData.categoryLabels.findIndex((l) => String(l) === strX) + 1;
		}
		const catIndex = w.labelData.labels.map((item) => Array.isArray(item) ? item.join(" ") : item).indexOf(x);
		const xLabel = w.dom.baseEl.querySelector(`.apexcharts-xaxis-texts-g text:nth-child(${catIndex + 1})`);
		if (xLabel) rX = parseFloat((_a = xLabel.getAttribute("x")) != null ? _a : "0");
		return rX;
	}
};
	...
	*/
	addXaxisAnnotation(anno, parent, index) {
		...
		if (anno.x2 === null || typeof anno.x2 === "undefined") {
			if (!clipX1) {
				const line = this.annoCtx.graphics.drawLine(x1 + anno.offsetX, 0 + anno.offsetY, x1 + anno.offsetX, w.layout.gridHeight + anno.offsetY, anno.borderColor, strokeDashArray, anno.borderWidth);
			...
			const rect = this.annoCtx.graphics.drawRect(x1 + anno.offsetX, 0 + anno.offsetY, x2 - x1, w.layout.gridHeight + anno.offsetY, 0, anno.fillColor, anno.opacity, 1, anno.borderColor, strokeDashArray);
		...
		if (!(clipX1 && clipX2)) {
			const textRects = this.annoCtx.graphics.getTextRects(text, anno.label.style.fontSize);
	...
	drawXAxisAnnotations() {
		const w = this.w;
		const elg = this.annoCtx.graphics.group({ class: "apexcharts-xaxis-annotations" });
...
var YAnnotations = class {
	...
	*/
	addYaxisAnnotation(anno, parent, index) {
		...
		if (anno.y2 === null || typeof anno.y2 === "undefined") {
			if (!clipY1) {
				drawn = true;
				const line = this.annoCtx.graphics.drawLine(0 + anno.offsetX, y1 + anno.offsetY, this._getYAxisAnnotationWidth(anno), y1 + anno.offsetY, anno.borderColor, strokeDashArray, anno.borderWidth);
				parent.appendChild(line.node);
				if (anno.id) line.node.classList.add(anno.id);
			}
		} else {
			...
			if (!(clipY1 && clipY2)) {
				drawn = true;
				const rect = this.annoCtx.graphics.drawRect(0 + anno.offsetX, y2 + anno.offsetY, this._getYAxisAnnotationWidth(anno), y1 - y2, 0, anno.fillColor, anno.opacity, 1, anno.borderColor, strokeDashArray);
				rect.node.classList.add("apexcharts-annotation-rect");
				rect.attr("clip-path", `url(#gridRectMask${w.globals.cuid})`);
				parent.appendChild(rect.node);
				if (anno.id) rect.node.classList.add(anno.id);
	...
	*/
	_getYAxisAnnotationWidth(anno) {
		...
		if (anno.width.indexOf("%") > -1) width = w.layout.gridWidth * parseInt(anno.width, 10) / 100;
		else width = parseInt(anno.width, 10);
		return width + anno.offsetX;
	}
	drawYAxisAnnotations() {
		const w = this.w;
		const elg = this.annoCtx.graphics.group({ class: "apexcharts-yaxis-annotations" });
		w.config.annotations.yaxis.forEach((anno, index) => {
			anno.yAxisIndex = this.axesUtils.translateYAxisIndex(anno.yAxisIndex);
			if (!(this.axesUtils.isYAxisHidden(anno.yAxisIndex) && this.axesUtils.yAxisAllSeriesCollapsed(anno.yAxisIndex))) this.addYaxisAnnotation(anno, elg.node, index);
		});
		return elg;
	}
};
var PointAnnotations = class {
	...
	*/
	addPointAnnotation(anno, parent, index) {
		if (this.w.globals.collapsedSeriesIndices.indexOf(anno.seriesIndex) > -1) return;
		...
		if (!(clipY || clipX)) {
			...
			if (anno.customSVG.SVG) {
				const g = this.annoCtx.graphics.group({ class: "apexcharts-point-annotations-custom-svg " + anno.customSVG.cssClass });
				g.attr({ transform: `translate(${x + anno.customSVG.offsetX}, ${y + anno.customSVG.offsetY})` });
	...
	drawPointAnnotations() {
		const w = this.w;
		const elg = this.annoCtx.graphics.group({ class: "apexcharts-point-annotations" });
...
var Annotations = class {
	...
	*/
		...
		if (w.globals.axisCharts && w.globals.dataPoints) {
			...
			for (let i = 0; i < 3; i++) {
				...
				if (initialAnim && !w.globals.resized && !w.globals.dataChanged) {
					if (w.config.chart.type !== "scatter" && w.config.chart.type !== "bubble" && w.globals.dataPoints > 1) annoElArray[i].classList.add("apexcharts-element-hidden");
				}
	...
	drawImageAnnos() {
		this.w.config.annotations.images.map((s) => {
			this.addImage(s);
		});
	}
	drawTextAnnos() {
		this.w.config.annotations.texts.map((t) => {
			this.addText(t);
		});
	}
	/**
	...
	*/
	addText(params) {
		const { x, y, text, textAnchor, foreColor, fontSize, fontFamily, fontWeight, cssClass, backgroundColor, borderWidth, strokeDashArray, borderRadius, borderColor, appendTo = ".apexcharts-svg", paddingLeft = 4, paddingRight = 4, paddingBottom = 2, paddingTop = 2 } = params;
		const w = this.w;
	...
	*/
	addImage(params) {
		const w = this.w;
		const { path, x = 0, y = 0, width = 20, height = 20, appendTo = ".apexcharts-svg" } = params;
	...
	*/
	...
	addAnnotationExternal({ params, pushToMemory, context, type, contextMethod }) {
		...
		const axesAnnoLabel = w.dom.baseEl.querySelector(`.apexcharts-${type}-annotations .apexcharts-${type}-annotation-label[rel='${index}']`);
	...
	*/
	clearAnnotations(ctx) {
		const w = ctx.w;
		const annos = w.dom.baseEl.querySelectorAll(".apexcharts-yaxis-annotations, .apexcharts-xaxis-annotations, .apexcharts-point-annotations");
		for (let i = w.globals.memory.methodsToExec.length - 1; i >= 0; i--) if (w.globals.memory.methodsToExec[i].label === "addText" || w.globals.memory.methodsToExec[i].label === "addAnnotation") w.globals.memory.methodsToExec.splice(i, 1);
	...
	removeAnnotation(ctx, id) {
		...
		if (annos) {
			...
			Object.keys(w.config.annotations).forEach((key) => {
				...
				if (Array.isArray(annotationArray)) w.config.annotations[key] = annotationArray.filter((m) => m.id !== id);
			});
			Array.prototype.forEach.call(annos, (a) => {
...
var KeyboardNavigation = class {
	...
	*/
	init() {
		const svgEl = this.w.dom.Paper.node;
		if (!svgEl) return;
	...
	*/
	_move(dSeries, dPoint) {
		...
		if (dSeries !== 0) {
			...
			if (ttCtx && ttCtx.tConfig && ttCtx.tConfig.shared) {
				...
				if (ttCtx.tooltipUtil && ttCtx.tooltipUtil.isXoverlap(j) && ttCtx.tooltipUtil.isInitialSeriesSameLen()) return;
			}
		...
		if (dPoint !== 0) {
			...
			if (dPoint > 0) this._skipNullForward();
			else this._skipNullBackward();
			if (!this._isDataPointVisible(this.seriesIndex, this.dataPointIndex)) this._snapToVisibleRangeInDirection(dPoint);
		}
		this._showCurrentPoint();
	}
	...
	_hideFocus() {
		...
		if (!ttCtx) return;
		if (ttCtx.marker) ttCtx.marker.resetPointsSize();
		this._enlargedScatterMarker = null;
		const tooltipEl = ttCtx.getElTooltip();
		if (tooltipEl) {
			tooltipEl.classList.remove("apexcharts-active");
			if (w.config.chart.accessibility.enabled && w.config.chart.accessibility.announcements.enabled) tooltipEl.setAttribute("aria-hidden", "true");
		}
		w.dom.baseEl.classList.remove("apexcharts-tooltip-active");
		const xcrosshairs = ttCtx.getElXCrosshairs();
	...
	*/
	_showTooltip(i, j, ttCtx) {
		...
		tooltipEl.classList.add("apexcharts-active");
		if (w.config.chart.accessibility.enabled && w.config.chart.accessibility.announcements.enabled) tooltipEl.removeAttribute("aria-hidden");
		if (type === "pie" || type === "donut" || type === "polarArea") this._showTooltipNonAxis(i, j, ttCtx, tooltipEl);
		else if (type === "radialBar") this._showTooltipRadialBar(i, j, ttCtx, tooltipEl);
		else if (type === "heatmap" || type === "treemap") this._showTooltipHeatTree(i, j, ttCtx, tooltipEl, type);
		else if (type === "bar" || type === "candlestick" || type === "boxPlot" || type === "rangeBar") this._showTooltipBar(i, j, ttCtx);
		else this._showTooltipAxisLine(i, j, ttCtx);
	}
	/**
	...
	*/
	_showTooltipBar(i, j, ttCtx) {
		...
		ttCtx.tooltipLabels.drawSeriesTexts(__spreadProps(__spreadValues(__spreadValues({
			...
			i,
			j
		}, (rangeData == null ? void 0 : rangeData.y1) !== void 0 && { y1: rangeData.y1 }), (rangeData == null ? void 0 : rangeData.y2) !== void 0 && { y2: rangeData.y2 }), { shared }));
		const parent = `.apexcharts-series[data\\:realIndex='${i}']`;
		const elPath = w.dom.Paper.findOne(`${parent} path[j='${j}'], ${parent} circle[j='${j}'], ${parent} rect[j='${j}']`);
		if (elPath) {
			this._leaveHoveredBar();
			new Graphics(this.w, this.ctx).pathMouseEnter(elPath, null);
		...
		if (w.globals.isBarHorizontal) {
			...
			if (barDomEl) {
				...
				let x = barCx + bw;
				if (barCx < (ttCtx.xyRatios && ttCtx.xyRatios.baseLineInvertedY != null ? ttCtx.xyRatios.baseLineInvertedY : wrapRect.width / 2)) x = barCx - ttWidth;
				const tooltipEl = ttCtx.getElTooltip();
				if (tooltipEl) {
					tooltipEl.style.left = x + "px";
					tooltipEl.style.top = y + "px";
		...
		} else ttCtx.tooltipPosition.moveStickyTooltipOverBars(j, i);
	}
	/**
	...
	*/
	_showTooltipAxisLine(i, j, ttCtx) {
		const w = this.w;
		const type = w.config.chart.type;
		const shared = ttCtx.tConfig.shared && ttCtx.tooltipUtil.isXoverlap(j) && ttCtx.tooltipUtil.isInitialSeriesSameLen();
		ttCtx.tooltipLabels.drawSeriesTexts({
			ttItems: ttCtx.ttItems,
			i,
			j,
		...
		if (isScatterLike) this._showScatterBubblePoint(i, j, ttCtx);
		else if (hasVisibleMarkers) if (shared) ttCtx.marker.enlargePoints(j);
		else ttCtx.tooltipPosition.moveDynamicPointOnHover(j, i);
		else if (shared) ttCtx.tooltipPosition.moveDynamicPointsOnHover(j);
		else ttCtx.tooltipPosition.moveDynamicPointOnHover(j, i);
	}
	/**
	...
	*/
	_showScatterBubblePoint(i, j, ttCtx) {
		...
		const seriesEl = baseEl.querySelector(`.apexcharts-series[data\\:realIndex='${i}']`);
		if (!seriesEl) return;
	...
	*/
	_showTooltipRadialBar(i, _j, ttCtx, tooltipEl) {
		...
		const arcEl = w.dom.baseEl.querySelector(`.apexcharts-radialbar-series[data\\:realIndex='${i}'] path`);
		if (arcEl) {
			const angle = parseFloat((_a = arcEl.getAttribute("data:angle")) != null ? _a : "") || 0;
			const midAngle = (w.config.plotOptions.radialBar.startAngle || 0) + angle / 2;
			...
			const ringRadius = (outerRadius + (outerRadius - trackSize)) / 2;
			const centroid = Utils$1.polarToCartesian(centerX, centerY, ringRadius, midAngle);
	...
	*/
	_showTooltipHeatTree(i, j, ttCtx, tooltipEl, type) {
		...
		if (cell) {
			...
			if (cellCx + cellWidth > w.layout.gridWidth / 2) x = cellCx - ttWidth / 2;
			tooltipEl.style.left = x + "px";
			tooltipEl.style.top = y + "px";
		}
	}
	...
	*/
	...
	_leaveHoveredBar() {
		if (this._hoveredBarEl) {
			new Graphics(this.w, this.ctx).pathMouseLeave(this._hoveredBarEl, null);
			this._hoveredBarEl = null;
		}
	}
	/**
	...
	*/
	_getFocusableElement(i, j) {
		...
		if (type === "pie" || type === "donut" || type === "polarArea") return baseEl.querySelector(`.apexcharts-pie-area[j='${j}']`);
		if (type === "heatmap") return baseEl.querySelector(`.apexcharts-heatmap-rect[i='${i}'][j='${j}']`);
		if (type === "treemap") return baseEl.querySelector(`.apexcharts-treemap-rect[i='${i}'][j='${j}']`);
		if (type === "radialBar") return baseEl.querySelector(`.apexcharts-radialbar-series[data\\:realIndex='${i}'] path`);
		if (type === "bar" || type === "candlestick" || type === "boxPlot" || type === "rangeBar") return baseEl.querySelector(`.apexcharts-series[data\\:realIndex='${i}'] path[j='${j}']`);
		return baseEl.querySelector(`.apexcharts-series[data\\:realIndex='${i}'] .apexcharts-marker[rel='${j}']`) || null;
	}
	_fireClick() {
		const ttCtx = this.w.globals.tooltip;
		if (!ttCtx) return;
		ttCtx.markerClick({
			...
			clientX: 0,
			clientY: 0
		}, this.seriesIndex, this.dataPointIndex);
	}
	_isNavEnabled() {
	...
	_getSeriesCount() {
		const w = this.w;
		const type = w.config.chart.type;
		if (type === "pie" || type === "donut" || type === "polarArea") return 1;
	...
	*/
	_getDataPointCount(si) {
		const w = this.w;
		const type = w.config.chart.type;
		if (type === "pie" || type === "donut" || type === "polarArea") return w.seriesData.series.length;
	...
	*/
	_snapToVisibleRange() {
		...
		for (let di = 0; di < dpCount; di++) if (seriesX[di] >= minX && seriesX[di] <= maxX) {
			this.dataPointIndex = di;
			return;
		}
	}
	...
	*/
	_snapToVisibleRangeInDirection(direction) {
		...
		} else for (let di = dpCount - 1; di >= 0; di--) if (seriesX[di] >= minX && seriesX[di] <= maxX) {
			this.dataPointIndex = di;
			return;
		}
	}
...
var BarDataLabels = class {
	...
	**/
	handleBarDataLabels(opts) {
		const { x, y, y1, y2, i, j, realIndex, columnGroupIndex, series, barHeight, barWidth, barXPosition, barYPosition, visibleSeries } = opts;
		const w = this.w;
		...
		if (w.config.dataLabels.enabled) {
			...
			textRects = graphics.getTextRects(w.config.dataLabels.formatter ? w.config.dataLabels.formatter(yLabel, __spreadProps(__spreadValues({}, w), {
				...
				dataPointIndex: j,
				w
			})) : w.formatters.yLabelFormatters[0](yLabel), parseFloat(dataLabelsConfig.style.fontSize).toString());
		}
		const params = {
			x,
			y,
		...
		if (w.config.chart.stacked && barTotalDataLabelsConfig.enabled) totalDataLabels = this.drawTotalDataLabels({
	...
	getStackedTotalDataLabel({ realIndex, j }) {
		...
		if (this.totalFormatter) val = this.totalFormatter(val, __spreadProps(__spreadValues({}, w), {
			seriesIndex: realIndex,
			dataPointIndex: j,
			w
		}));
	...
	*/
	calculateColumnsDataLabelsPosition(opts) {
		const w = this.w;
		let { i, j, realIndex, y, bcx, barWidth, barHeight, textRects, dataLabelsX, dataLabelsY, dataLabelsConfig, barDataLabelsConfig, barTotalDataLabelsConfig, strokeWidth, offX, offY } = opts;
		...
		if (this.barCtx.isVerticalGroupedRangeBar) dataLabelsX += barWidth / 2;
		else {
			if (w.axisFlags.isXNumeric) dataLabelsX = bcx - barWidth / 2 + offX;
			else dataLabelsX = bcx - dataPointsDividedWidth + barWidth / 2 + offX;
			if (!w.config.chart.stacked && zeroEncounters > 0 && w.config.plotOptions.bar.hideZeroBarsWhenGrouped) dataLabelsX -= barWidth * zeroEncounters;
		}
		if (vertical) dataLabelsX = dataLabelsX + textRects.height / 2 - strokeWidth / 2 - 2;
		const valIsNegative = w.seriesData.series[i][j] < 0;
		let newY = y;
		if (this.barCtx.isReversed) newY = y + (valIsNegative ? barHeight : -barHeight);
		switch (barDataLabelsConfig.position) {
			case "center":
				if (vertical) if (valIsNegative) dataLabelsY = newY - barHeight / 2 + offY;
				else dataLabelsY = newY + barHeight / 2 - offY;
				else if (valIsNegative) dataLabelsY = newY - barHeight / 2 + textRects.height / 2 + offY;
				else dataLabelsY = newY + barHeight / 2 + textRects.height / 2 - offY;
				break;
			case "bottom":
				if (vertical) if (valIsNegative) dataLabelsY = newY - barHeight + offY;
				else dataLabelsY = newY + barHeight - offY;
				else if (valIsNegative) dataLabelsY = newY - barHeight + textRects.height + strokeWidth + offY;
				else dataLabelsY = newY + barHeight - textRects.height / 2 + strokeWidth - offY;
				break;
			case "top":
				if (vertical) if (valIsNegative) dataLabelsY = newY + offY;
				else dataLabelsY = newY - offY;
				else if (valIsNegative) dataLabelsY = newY - textRects.height / 2 - offY;
		...
		w.labelData.seriesGroups.forEach((sg) => {
			var _a;
			(_a = this.barCtx[sg.join(",")]) == null || _a.prevY.forEach((arr) => {
				if (valIsNegative) lowestPrevY = Math.max(arr[j], lowestPrevY);
				else lowestPrevY = Math.min(arr[j], lowestPrevY);
			});
		});
		if (this.barCtx.lastActiveBarSerieIndex === realIndex && barTotalDataLabelsConfig.enabled) {
			const ADDITIONAL_OFFY = 18;
			const totalLabeltextRects = new Graphics(this.barCtx.w).getTextRects(this.getStackedTotalDataLabel({
				realIndex,
				j
			}), dataLabelsConfig.fontSize);
			if (valIsNegative) totalDataLabelsY = lowestPrevY - totalLabeltextRects.height / 2 - offY - barTotalDataLabelsConfig.offsetY + ADDITIONAL_OFFY;
		...
		if (!w.config.chart.stacked) {
			...
			else if (dataLabelsY + textRects.height / 3 > w.layout.gridHeight) dataLabelsY = w.layout.gridHeight - strokeWidth;
		}
		return {
			bcx,
			bcy: y,
	...
	*/
	calculateBarsDataLabelsPosition(opts) {
		const w = this.w;
		let { x, i, j, realIndex, bcy, barHeight, barWidth, textRects, dataLabelsX, strokeWidth, dataLabelsConfig, barDataLabelsConfig, barTotalDataLabelsConfig, offX, offY } = opts;
		...
		if (!w.config.chart.stacked && zeroEncounters > 0 && w.config.plotOptions.bar.hideZeroBarsWhenGrouped) dataLabelsY -= barHeight * zeroEncounters;
		let totalDataLabelsX;
		let totalDataLabelsY;
		...
		switch (barDataLabelsConfig.position) {
			case "center":
				...
				break;
			case "bottom":
				if (valIsNegative) dataLabelsX = newX + barWidth - strokeWidth - offX;
				else dataLabelsX = newX - barWidth + strokeWidth + offX;
				break;
			case "top":
				if (valIsNegative) dataLabelsX = newX - strokeWidth - offX;
				else dataLabelsX = newX - strokeWidth + offX;
				break;
		}
		let lowestPrevX = newX;
		w.labelData.seriesGroups.forEach((sg) => {
			var _a;
			(_a = this.barCtx[sg.join(",")]) == null || _a.prevX.forEach((arr) => {
				if (valIsNegative) lowestPrevX = Math.min(arr[j], lowestPrevX);
				else lowestPrevX = Math.max(arr[j], lowestPrevX);
			});
		});
		if (this.barCtx.lastActiveBarSerieIndex === realIndex && barTotalDataLabelsConfig.enabled) {
			const totalLabeltextRects = new Graphics(this.barCtx.w).getTextRects(this.getStackedTotalDataLabel({
			...
			} else totalDataLabelsX = lowestPrevX + offX + barTotalDataLabelsConfig.offsetX + (this.barCtx.isReversed ? -(barWidth + strokeWidth) : strokeWidth);
			...
			if (w.globals.barGroups.length > 1) totalDataLabelsY = totalDataLabelsY - w.globals.barGroups.length / 2 * (barHeight / 2);
		}
		if (!w.config.chart.stacked) {
			if (dataLabelsConfig.textAnchor === "start") {
				if (dataLabelsX - textRects.width < 0) dataLabelsX = valIsNegative ? textRects.width + strokeWidth : strokeWidth;
				else if (dataLabelsX + textRects.width > w.layout.gridWidth) dataLabelsX = valIsNegative ? w.layout.gridWidth - strokeWidth : w.layout.gridWidth - textRects.width - strokeWidth;
			} else if (dataLabelsConfig.textAnchor === "middle") {
				if (dataLabelsX - textRects.width / 2 < 0) dataLabelsX = textRects.width / 2 + strokeWidth;
				else if (dataLabelsX + textRects.width / 2 > w.layout.gridWidth) dataLabelsX = w.layout.gridWidth - textRects.width / 2 - strokeWidth;
			} else if (dataLabelsConfig.textAnchor === "end") {
				if (dataLabelsX < 1) dataLabelsX = textRects.width + strokeWidth;
				else if (dataLabelsX + 1 > w.layout.gridWidth) dataLabelsX = w.layout.gridWidth - textRects.width - strokeWidth;
			}
		}
		return {
			bcx: x,
	...
	drawCalculatedDataLabels({ x, y, val, i, j, textRects, barHeight, barWidth, dataLabelsConfig }) {
		const w = this.w;
		let rotate = "rotate(0)";
		...
		if (dataLabelsConfig.enabled && !isSeriesNotCollapsed) {
			...
			let text = "";
			if (typeof val !== "undefined") text = formatter(val, __spreadProps(__spreadValues({}, w), {
				seriesIndex: i,
				dataPointIndex: j,
				w
			}));
			...
			if (w.config.plotOptions.bar.dataLabels.orientation === "vertical") {
				if (position === "top") if (valIsNegative) dataLabelsConfig.textAnchor = "end";
				else dataLabelsConfig.textAnchor = "start";
				if (position === "center") dataLabelsConfig.textAnchor = "middle";
				if (position === "bottom") if (valIsNegative) dataLabelsConfig.textAnchor = "end";
				else dataLabelsConfig.textAnchor = "start";
			}
			if (this.barCtx.isRangeBar && this.barCtx.barOptions.dataLabels.hideOverflowingLabels) {
				if (barWidth < graphics.getTextRects(text, parseFloat(dataLabelsConfig.style.fontSize).toString()).width) text = "";
			}
			if (w.config.chart.stacked && this.barCtx.barOptions.dataLabels.hideOverflowingLabels) {
				...
				} else if (textRects.height / 1.6 > Math.abs(barHeight)) text = "";
			}
	...
	drawTotalDataLabels({ x, y, val, realIndex, textAnchor, barTotalDataLabelsConfig }) {
		const graphics = new Graphics(this.barCtx.w);
		let totalDataLabelText;
		if (barTotalDataLabelsConfig.enabled && typeof x !== "undefined" && typeof y !== "undefined" && this.barCtx.lastActiveBarSerieIndex === realIndex) totalDataLabelText = graphics.drawText({
			x,
			y,
...
var Helpers$1 = class Helpers4 {
	...
	*/
	initVariables(series) {
		...
		for (let sl = 0; sl < series.length; sl++) {
			...
			if (w.axisFlags.isXNumeric) {
				for (let j = 0; j < series[sl].length; j++) if (w.seriesData.seriesX[sl][j] > w.globals.minX && w.seriesData.seriesX[sl][j] < w.globals.maxX) this.barCtx.visibleItems++;
			} else this.barCtx.visibleItems = w.globals.dataPoints;
		}
		this.arrBorderRadius = this.createBorderRadiusArr(w.seriesData.series);
		if (Utils$1.isSafari()) this.arrBorderRadius = this.arrBorderRadius.map((brArr) => brArr.map((_) => "none"));
		if (this.barCtx.seriesLen === 0) this.barCtx.seriesLen = 1;
		this.barCtx.zeroSerieses = [];
		if (!w.globals.comboCharts) this.checkZeroSeries({ series });
	}
	/**
	* @param {number} realIndex
	*/
	initialPositions(realIndex) {
		...
		if (this.barCtx.isRangeBar) dataPoints = w.labelData.labels.length;
		...
		if (this.barCtx.isHorizontal) {
			...
			if (this.barCtx.isFunnel) zeroW = w.layout.gridWidth / 2;
			y = (yDivision - barHeight * this.barCtx.seriesLen) / 2;
		} else {
			xDivision = w.layout.gridWidth / this.barCtx.visibleItems;
			if (w.config.xaxis.convertedCatToNumeric) xDivision = w.layout.gridWidth / w.globals.dataPoints;
			barWidth = xDivision / seriesLen * parseInt(this.barCtx.barOptions.columnWidth, 10) / 100;
			if (w.axisFlags.isXNumeric) {
				const xRatio = this.barCtx.xRatio;
				if (w.globals.minXDiff && w.globals.minXDiff !== .5 && w.globals.minXDiff / xRatio > 0) xDivision = w.globals.minXDiff / xRatio;
				barWidth = xDivision / seriesLen * parseInt(this.barCtx.barOptions.columnWidth, 10) / 100;
				if (barWidth < 1) barWidth = 1;
			}
			...
			if (w.axisFlags.isXNumeric) x = this.barCtx.getBarXForNumericXAxis({
				x,
				j: 0,
				realIndex,
				barWidth
			}).x;
			else x = w.globals.padHorizontal + Utils$1.noExponents(xDivision - barWidth * this.barCtx.seriesLen) / 2;
		}
	...
	*/
	initializeStackedPrevVars(ctx) {
		ctx.w.labelData.seriesGroups.forEach((group) => {
	...
	*/
	getPathFillColor(series, i, j, realIndex) {
		...
		if (this.barCtx.barOptions.colors.ranges.length > 0) this.barCtx.barOptions.colors.ranges.map((range) => {
	...
	*/
	getStrokeWidth(i, j, realIndex) {
		let strokeWidth = 0;
		const w = this.w;
		if (typeof this.barCtx.series[i][j] === "undefined" || this.barCtx.series[i][j] === null || w.config.chart.type === "bar" && !this.barCtx.series[i][j]) this.barCtx.isNullValue = true;
		else this.barCtx.isNullValue = false;
	...
	*/
	createBorderRadiusArr(series) {
		...
		const output = Array.from({ length: numSeries }, () => Array(numColumns).fill(alwaysApplyRadius ? "top" : "none"));
		...
		for (let j = 0; j < numColumns; j++) {
			...
			if (positiveIndices.length > 0 && negativeIndices.length === 0) if (positiveIndices.length === 1) output[positiveIndices[0]][j] = chartType === "bar" && numColumns === 1 ? "top" : "both";
			else {
				const firstPositiveIndex = positiveIndices[0];
				const lastPositiveIndex = positiveIndices[positiveIndices.length - 1];
				for (const i of positiveIndices) if (i === firstPositiveIndex) output[i][j] = chartType === "bar" && numColumns === 1 ? "top" : "bottom";
				else if (i === lastPositiveIndex) output[i][j] = "top";
				else output[i][j] = "none";
			}
			else if (negativeIndices.length > 0 && positiveIndices.length === 0) if (negativeIndices.length === 1) output[negativeIndices[0]][j] = "both";
			else {
				const highestNegativeIndex = Math.max(...negativeIndices);
				const lowestNegativeIndex = Math.min(...negativeIndices);
				for (const i of negativeIndices) if (i === highestNegativeIndex) output[i][j] = "bottom";
				else if (i === lowestNegativeIndex) output[i][j] = "top";
				else output[i][j] = "none";
			}
			else if (positiveIndices.length > 0 && negativeIndices.length > 0) {
				const lastPositiveIndex = positiveIndices[positiveIndices.length - 1];
				for (const i of positiveIndices) if (i === lastPositiveIndex) output[i][j] = "top";
				else output[i][j] = "none";
				const highestNegativeIndex = Math.max(...negativeIndices);
				for (const i of negativeIndices) if (i === highestNegativeIndex) output[i][j] = "bottom";
				else output[i][j] = "none";
	...
	barBackground({ j, i, x1, x2, y1, y2, elSeries }) {
		const w = this.w;
		const graphics = new Graphics(this.barCtx.w);
		const activeSeriesIndex = new Series(this.barCtx.w).getActiveConfigSeriesIndex();
		if (this.barCtx.barOptions.colors.backgroundBarColors.length > 0 && activeSeriesIndex === i) {
			...
			const rect = graphics.drawRect(typeof x1 !== "undefined" ? x1 : 0, typeof y1 !== "undefined" ? y1 : 0, typeof x2 !== "undefined" ? x2 : w.layout.gridWidth, typeof y2 !== "undefined" ? y2 : w.layout.gridHeight, this.barCtx.barOptions.colors.backgroundBarRadius, bcolor, this.barCtx.barOptions.colors.backgroundBarOpacity);
			elSeries.add(rect);
	...
	getColumnPaths({ barWidth, barXPosition, y1, y2, strokeWidth, isReversed, series, seriesGroup, realIndex, i, j, w }) {
		var _a;
		const graphics = new Graphics(this.barCtx.w);
		...
		y1 += .001 - strokeCenter * direction;
		y2 += .001 + strokeCenter * direction;
		...
		if (this.arrBorderRadius[realIndex][j] !== "none") pathTo = graphics.roundPathCorners(pathTo, w.config.plotOptions.bar.borderRadius);
		if (w.config.chart.stacked) {
	...
	getBarpaths({ barYPosition, barHeight, x1, x2, strokeWidth, isReversed, series, seriesGroup, realIndex, i, j, w }) {
		var _a;
		const graphics = new Graphics(this.barCtx.w);
		strokeWidth = Array.isArray(strokeWidth) ? strokeWidth[realIndex] : strokeWidth;
		if (!strokeWidth) strokeWidth = 0;
		...
		const direction = (series[i][j] >= 0 ? 1 : -1) * (isReversed ? -1 : 1);
		x1 += .001 + strokeCenter * direction;
		x2 += .001 - strokeCenter * direction;
		let pathTo = graphics.move(x1, y1);
		let pathFrom = graphics.move(x1, y1);
		if (w.globals.previousPaths.length > 0) pathFrom = this.barCtx.getPreviousPath(realIndex, j, false);
	...
	checkZeroSeries({ series }) {
		...
		for (let zs = 0; zs < series.length; zs++) {
			let total = 0;
			for (let zsj = 0; zsj < series[w.globals.maxValsInArrayIndex].length; zsj++) total += series[zs][zsj];
	...
	*/
	getYForValue(value, zeroH, translationsIndex, zeroPositionForNull = true) {
		...
		if (typeof value !== "undefined" && value !== null) yForVal = zeroH - value / this.barCtx.yRatio[translationsIndex] + (this.barCtx.isReversed ? value / this.barCtx.yRatio[translationsIndex] : 0) * 2;
		return yForVal;
	}
	/**
	* @param {string} type
	...
	*/
	getGoalValues(type, zeroW, zeroH, i, j, translationsIndex) {
		...
		if (w.seriesData.seriesGoals[i] && w.seriesData.seriesGoals[i][j] && Array.isArray(w.seriesData.seriesGoals[i][j])) w.seriesData.seriesGoals[i][j].forEach((goal) => {
		...
		if (this.barCtx.barOptions.isDumbbell && w.rangeData.seriesRange.length) {
			...
			pushGoal(w.rangeData.seriesRangeEnd[i][j], __spreadProps(__spreadValues({}, commonAttrs), { strokeColor: Array.isArray(colors[i]) ? colors[i][1] : colors[i] }));
		}
		return goals;
	}
	/** @param {{barXPosition: any, barYPosition: any, goalX: any, goalY: any, barWidth: any, barHeight: any}} opts */
	drawGoalLine({ barXPosition, barYPosition, goalX, goalY, barWidth, barHeight }) {
		const graphics = new Graphics(this.barCtx.w);
		const lineGroup = graphics.group({ className: "apexcharts-bar-goals-groups" });
		...
		let line = null;
		if (this.barCtx.isHorizontal) {
			if (Array.isArray(goalX)) goalX.forEach((goal) => {
		...
		} else if (Array.isArray(goalY)) goalY.forEach((goal) => {
	...
	drawBarShadow({ prevPaths, currPaths, color, realIndex, j }) {
		...
		return graphics.drawPath({
			d: shadowPath,
			fill: utils.shadeColor(.5, Utils$1.rgb2hex(color)),
			stroke: "none",
			strokeWidth: 0,
			fillOpacity: 1,
	...
	getZeroValueEncounters({ i, j }) {
		...
		(w.config.plotOptions.bar.horizontal ? w.seriesData.series.map((_, _i) => _i) : ((_a = w.globals.columnSeries) == null ? void 0 : _a.i.map((_i) => _i)) || []).forEach((_si) => {
			const val = w.globals.seriesPercent[_si][j];
			if (val) nonZeroColumns++;
			if (_si < i && val === 0) zeroEncounters++;
		});
		return {
			nonZeroColumns,
			zeroEncounters
	...
	*/
	getGroupIndex(seriesIndex) {
		const w = this.w;
		const groupIndex = w.labelData.seriesGroups.findIndex((group) => group.indexOf(w.seriesData.seriesNames[seriesIndex]) > -1);
...
var Bar = class {
	...
	*/
	constructor(w, ctx, xyRatios) {
		...
		this.stackedSeriesTotals = new CoreUtils(this.w).getStackedSeriesTotals(this.w.config.series.map((s, i) => {
	...
	**/
	draw(series, seriesIndex) {
		...
		const ret = graphics.group({ class: "apexcharts-bar-series apexcharts-plot-series" });
		if (w.config.dataLabels.enabled) {
			if (this.totalItems > this.barOptions.dataLabels.maxItems) console.warn("WARNING: DataLabels are enabled but there are too many to display. This may cause performance issue when rendering - ApexCharts");
		}
		for (let i = 0, bc = 0; i < series.length; i++, bc++) {
			...
			const { y: initY, yDivision, zeroW, x: initX, xDivision, zeroH } = initPositions;
			let barHeight = initPositions.barHeight;
			let barWidth = initPositions.barWidth;
			y = initY;
			x = initX;
			if (!this.isHorizontal) xArrj.push(x + (barWidth != null ? barWidth : 0) / 2);
			const elDataLabelsWrap = graphics.group({
				class: "apexcharts-datalabels",
				"data:realIndex": realIndex
			});
			w.globals.delayedElements.push({ el: elDataLabelsWrap.node });
			elDataLabelsWrap.node.classList.add("apexcharts-element-hidden");
			const elGoalsMarkers = graphics.group({ class: "apexcharts-bar-goals-markers" });
			const elBarShadows = graphics.group({ class: "apexcharts-bar-shadows" });
			w.globals.delayedElements.push({ el: elBarShadows.node });
			...
			for (let j = 0; j < series[i].length; j++) {
				...
				if (this.isFunnel && this.barOptions.isFunnel3d && this.pathArr.length && j > 0) {
					...
					if (w.config.chart.dropShadow.enabled) new Filters(this.w).dropShadow(barShadow, w.config.chart.dropShadow, realIndex);
				}
				...
				y = paths.y;
				x = paths.x;
				if (j > 0) xArrj.push(x + (barWidth != null ? barWidth : 0) / 2);
				yArrj.push(y);
	...
	renderSeries({ realIndex, pathFill, lineFill, j, i, columnGroupIndex, pathFrom, pathTo, strokeWidth, elSeries, x, y, y1, y2, series, barHeight, barWidth, barXPosition, barYPosition, elDataLabelsWrap, elGoalsMarkers, elBarShadows, visibleSeries, type, classes }) {
		const w = this.w;
		const graphics = new Graphics(this.w, this.ctx);
		...
		const dataLabelsObj = new BarDataLabels(this).handleBarDataLabels({
			x,
			y,
			y1,
			y2,
		...
		if (!w.globals.isBarHorizontal) {
			if (dataLabelsObj.dataLabelsPos.dataLabelsX + Math.max(barWidth, w.globals.barPadForNumericAxis) < 0 || dataLabelsObj.dataLabelsPos.dataLabelsX - Math.max(barWidth, w.globals.barPadForNumericAxis) > w.layout.gridWidth) skipDrawing = true;
		}
		...
		if (!skipDrawing) {
			...
			new Filters(this.w).setSelectionFilter(renderedPath, realIndex, j);
			...
			if (elBarShadows) elSeries.add(elBarShadows);
		}
		return elSeries;
	}
	/** @param {{indexes: any, barHeight: any, strokeWidth: any, zeroW: any, x: any, y: any, yDivision: any, elSeries: any}} opts */
	drawBarPaths({ indexes, barHeight, strokeWidth, zeroW, x, y, yDivision, elSeries }) {
		const w = this.w;
		const i = indexes.i;
		const j = indexes.j;
		let barYPosition;
		...
		} else if (w.config.plotOptions.bar.hideZeroBarsWhenGrouped) {
			const { nonZeroColumns, zeroEncounters } = this.barHelpers.getZeroValueEncounters({
				i,
				j
			});
		...
		if (this.isFunnel) {
			...
			zeroW = _zeroW - (this.barHelpers.getXForValue(
				/** @type {any} */
				this.series[i][j],
				_zeroW
			) - _zeroW) / 2;
		...
		if (!w.axisFlags.isXNumeric) y = y + yDivision;
	...
	drawColumnPaths({ indexes, x, y, xDivision, barWidth, zeroH, strokeWidth, elSeries }) {
		const w = this.w;
		...
		if (!w.axisFlags.isXNumeric) x = x + xDivision;
		this.barHelpers.barBackground({
			bc,
			j,
			i,
	...
	getBarXForNumericXAxis({ x, barWidth, realIndex, j }) {
		const w = this.w;
		let sxI = realIndex;
		if (!w.seriesData.seriesX[realIndex].length) sxI = w.globals.maxValsInArrayIndex;
		if (Utils$1.isNumber(w.seriesData.seriesX[sxI][j])) x = (w.seriesData.seriesX[sxI][j] - w.globals.minX) / this.xRatio - barWidth * this.seriesLen / 2;
		return {
			barXPosition: x + barWidth * this.visibleI,
			x
		};
...
var BarStacked = class extends Bar {
	...
	*/
	draw(series, seriesIndex) {
		...
		this.yRatio = coreUtils.getLogYRatios(this.yRatio);
		this.barHelpers.initVariables(series);
		if (w.config.chart.stackType === "100%") series = w.globals.comboCharts ? seriesIndex.map((_) => w.globals.seriesPercent[_]) : w.globals.seriesPercent.slice();
		this.series = series;
		this.barHelpers.initializeStackedPrevVars(this);
		const ret = this.graphics.group({ class: "apexcharts-bar-series apexcharts-plot-series" });
		let x = 0;
		let y = 0;
		for (let i = 0, bc = 0; i < series.length; i++, bc++) {
			const realIndex = w.globals.comboCharts ? seriesIndex[i] : i;
			...
			const elGoalsMarkers = this.graphics.group({ class: "apexcharts-bar-goals-markers" });
			const initPositions = this.initialPositions(x, y, void 0, void 0, void 0, void 0, translationsIndex);
			const { xDivision, yDivision, zeroH, zeroW } = initPositions;
			let barHeight = initPositions.barHeight;
			let barWidth = initPositions.barWidth;
			y = initPositions.y;
			x = initPositions.x;
			...
			for (let j = 0; j < w.globals.dataPoints; j++) {
				...
				if (this.barHelpers.arrBorderRadius[realIndex][j] === "bottom" && w.seriesData.series[realIndex][j] > 0 || this.barHelpers.arrBorderRadius[realIndex][j] === "top" && w.seriesData.series[realIndex][j] < 0) classes = flipClass;
	...
	*/
	initialPositions(x, y, xDivision, yDivision, zeroH, zeroW, translationsIndex) {
		...
		if (this.isHorizontal) {
			...
			if (String(userBarHeight).indexOf("%") === -1) barHeight = parseInt(userBarHeight, 10);
			else barHeight = yDivision * parseInt(userBarHeight, 10) / 100;
			...
			} else if (String(userColumnWidth).indexOf("%") === -1) barWidth = parseInt(userColumnWidth, 10);
			else barWidth *= parseInt(userColumnWidth, 10) / 100;
			if (this.isReversed) zeroH = this.baseLineY[translationsIndex];
			else zeroH = w.layout.gridHeight - this.baseLineY[translationsIndex];
	...
	drawStackedBarPaths({ indexes, barHeight, strokeWidth, zeroW, x, y, columnGroupIndex, seriesGroup, yDivision, elSeries }) {
		var _a, _b, _c, _d, _e;
		const w = this.w;
		...
		const translationsIndex = indexes.translationsIndex;
		let prevBarW = 0;
		for (let k = 0; k < this.groupCtx.prevXF.length; k++) prevBarW = prevBarW + this.groupCtx.prevXF[k][j];
		let gsi = i;
		if (w.config.series[realIndex].name) gsi = seriesGroup.indexOf(
			/** @type {Record<string,any>} */
			w.config.series[realIndex].name
		);
		if (gsi > 0) {
			let bXP = zeroW;
			if (this.groupCtx.prevXVal[gsi - 1][j] < 0) bXP = ((_a = this.series[i]) == null ? void 0 : _a[j]) >= 0 ? this.groupCtx.prevX[gsi - 1][j] + prevBarW - (this.isReversed ? prevBarW : 0) * 2 : this.groupCtx.prevX[gsi - 1][j];
			else if (this.groupCtx.prevXVal[gsi - 1][j] >= 0) bXP = ((_b = this.series[i]) == null ? void 0 : _b[j]) >= 0 ? this.groupCtx.prevX[gsi - 1][j] : this.groupCtx.prevX[gsi - 1][j] - prevBarW + (this.isReversed ? prevBarW : 0) * 2;
			barXPosition = bXP;
		} else barXPosition = zeroW;
		if (((_c = this.series[i]) == null ? void 0 : _c[j]) === null) x = barXPosition;
		else x = barXPosition + ((_d = this.series[i]) == null ? void 0 : _d[j]) / this.invertedYRatio - (this.isReversed ? ((_e = this.series[i]) == null ? void 0 : _e[j]) / this.invertedYRatio : 0) * 2;
		const paths = this.barHelpers.getBarpaths({
			barYPosition,
			barHeight,
			x1: barXPosition,
	...
	drawStackedColumnPaths({ indexes, x, y, xDivision, barWidth, zeroH, columnGroupIndex, seriesGroup, elSeries }) {
		var _a, _b, _c, _d, _e, _f, _g, _h, _i;
		const w = this.w;
		const i = indexes.i;
		const j = indexes.j;
		...
		let barYPosition;
		let prevBarH = 0;
		for (let k = 0; k < this.groupCtx.prevYF.length; k++) prevBarH = prevBarH + (!isNaN(this.groupCtx.prevYF[k][j]) ? this.groupCtx.prevYF[k][j] : 0);
		let gsi = i;
		if (seriesGroup) gsi = seriesGroup.indexOf(w.seriesData.seriesNames[realIndex]);
		if (gsi > 0 && !w.axisFlags.isXNumeric || gsi > 0 && w.axisFlags.isXNumeric && w.seriesData.seriesX[realIndex - 1][j] === w.seriesData.seriesX[realIndex][j]) {
			...
			if (this.groupCtx.prevY[gsi - 1] !== void 0 && this.groupCtx.prevY[gsi - 1].length) {
				for (let ii = 1; ii < p; ii++) if (!isNaN((_a = this.groupCtx.prevY[gsi - ii]) == null ? void 0 : _a[j])) {
					prevYValue = this.groupCtx.prevY[gsi - ii][j];
					break;
				}
			}
			for (let ii = 1; ii < p; ii++) if (((_b = this.groupCtx.prevYVal[gsi - ii]) == null ? void 0 : _b[j]) < 0) {
				bYP = ((_c = this.series[i]) == null ? void 0 : _c[j]) >= 0 ? prevYValue - prevBarH + (this.isReversed ? prevBarH : 0) * 2 : prevYValue;
				break;
			} else if (((_d = this.groupCtx.prevYVal[gsi - ii]) == null ? void 0 : _d[j]) >= 0) {
				bYP = ((_e = this.series[i]) == null ? void 0 : _e[j]) >= 0 ? prevYValue : prevYValue + prevBarH - (this.isReversed ? prevBarH : 0) * 2;
				break;
			}
			if (typeof bYP === "undefined") bYP = w.layout.gridHeight;
			if (((_f = this.groupCtx.prevYF[0]) == null ? void 0 : _f.every((val) => val === 0)) && this.groupCtx.prevYF.slice(1, gsi).every((arr) => arr.every((val) => isNaN(val)))) barYPosition = zeroH;
			else barYPosition = bYP;
		} else barYPosition = zeroH;
		if ((_g = this.series[i]) == null ? void 0 : _g[j]) y = barYPosition - ((_h = this.series[i]) == null ? void 0 : _h[j]) / this.yRatio[translationsIndex] + (this.isReversed ? ((_i = this.series[i]) == null ? void 0 : _i[j]) / this.yRatio[translationsIndex] : 0) * 2;
		else y = barYPosition;
		const paths = this.barHelpers.getColumnPaths({
			barXPosition,
			barWidth,
...
var BoxCandleStick = class extends Bar {
	...
	*/
	draw(series, ctype, seriesIndex) {
		...
		this.barHelpers.initVariables(series);
		const ret = graphics.group({ class: `apexcharts-${type}-series apexcharts-plot-series` });
		for (let i = 0; i < series.length; i++) {
			...
			const { y: initY, barHeight, yDivision, zeroW, x: initX, barWidth, xDivision, zeroH } = this.barHelpers.initialPositions(realIndex);
			y = initY;
			x = initX;
			...
			for (let j = 0; j < w.globals.dataPoints; j++) {
				...
				if (this.isHorizontal) paths = this.drawHorizontalBoxPaths(__spreadProps(__spreadValues({}, pathsParams), {
					yDivision,
					barHeight,
					zeroW
				}));
				else paths = this.drawVerticalBoxPaths(__spreadProps(__spreadValues({}, pathsParams), {
					xDivision,
					barWidth,
					zeroH
				}));
	...
	drawVerticalBoxPaths({ indexes, x, xDivision, barWidth, zeroH, strokeWidth }) {
		var _a, _b;
		const w = this.w;
		const graphics = new Graphics(this.w);
		const i = indexes.i;
		...
		if (this.isBoxPlot) color = [getColor(boxColors.lower), getColor(boxColors.upper)];
		let y1 = Math.min(ohlc.o, ohlc.c);
		let y2 = Math.max(ohlc.o, ohlc.c);
		let m = ohlc.m;
		if (w.axisFlags.isXNumeric) x = (w.seriesData.seriesX[realIndex][j] - w.globals.minX) / this.xRatio - barWidth / 2;
		const barXPosition = x + barWidth * this.visibleI;
		if (typeof ((_a = this.series[i]) == null ? void 0 : _a[j]) === "undefined" || ((_b = this.series[i]) == null ? void 0 : _b[j]) === null) {
			y1 = zeroH;
			y2 = zeroH;
		} else {
			y1 = zeroH - y1 / yRatio;
		...
		if (w.globals.previousPaths.length > 0) pathFrom = this.getPreviousPath(realIndex, j);
		if (this.isOHLC) {
	...
	drawHorizontalBoxPaths({ indexes, y, yDivision, barHeight, zeroW, strokeWidth }) {
		var _a, _b;
		const w = this.w;
		const graphics = new Graphics(this.w);
		const i = indexes.i;
		...
		let m = ohlc.m;
		if (w.axisFlags.isXNumeric) y = (w.seriesData.seriesX[realIndex][j] - w.globals.minX) / this.invertedXRatio - barHeight / 2;
...
var TreemapHelpers = class {
	...
	*/
	...
	checkColorRange() {
		...
		if (chartOpts.colorScale.ranges.length > 0) chartOpts.colorScale.ranges.map((range) => {
	...
	*/
	getShadeColor(chartType, i, j, negRange) {
		...
		if (w.globals.hasNegs || negRange) if (w.config.plotOptions[chartType].reverseNegativeShade) if (colorProps.percent < 0) colorShadePercent = colorProps.percent / 100 * (shadeIntensity * 1.25);
		else colorShadePercent = (1 - colorProps.percent / 100) * (shadeIntensity * 1.25);
		else if (colorProps.percent <= 0) colorShadePercent = 1 - (1 + colorProps.percent / 100) * shadeIntensity;
		...
		const utils = new Utils$1();
		if (w.config.plotOptions[chartType].enableShades) if (this.w.config.theme.mode === "dark") {
	...
	*/
	...
	calculateDataLabels({ text, x, y, i, j, colorProps, fontSize }) {
		const dataLabelsConfig = this.w.config.dataLabels;
		const graphics = new Graphics(this.w);
...
var HeatMap = class {
	...
	*/
	draw(series) {
		const w = this.w;
		const graphics = new Graphics(this.w, this.ctx);
		const ret = graphics.group({ class: "apexcharts-heatmap" });
		...
		for (let i = rev ? 0 : heatSeries.length - 1; rev ? i < heatSeries.length : i >= 0; rev ? i++ : i--) {
			...
			if (w.config.chart.dropShadow.enabled) {
				...
				new Filters(this.w).dropShadow(elSeries, shadow, i);
			}
			let x1 = 0;
			...
			for (let dIndex = 0; dIndex < w.globals.dataPoints; dIndex++) {
				...
				const heatColor = this.helpers.getShadeColor(w.config.chart.type, i, j, this.negRange);
				let color = heatColor.color;
				const heatColorProps = heatColor.colorProps;
				if (w.config.fill.type === "image") color = new Fill(this.w).fillPath({
					seriesNumber: i,
					dataPointIndex: j,
	...
	*/
	sameValueSeriesFix(i, series) {
		...
		if (w.config.fill.type === "gradient" || w.config.fill.type[i] === "gradient") {
			if (new CoreUtils(this.lineCtx.w).seriesHaveSameValues(i)) {
	...
	calculatePoints({ series, realIndex, x, y, i, j, prevY }) {
		...
		if (w.axisFlags.isXNumeric) xPT1st = (w.seriesData.seriesX[realIndex][0] - w.globals.minX) / this.lineCtx.xRatio + w.config.markers.offsetX;
		if (j === 0) {
			ptX.push(xPT1st);
	...
	determineFirstPrevY({ i, realIndex, series, prevY, lineYPosition, translationsIndex }) {
		...
		if (typeof ((_c = series[i]) == null ? void 0 : _c[0]) !== "undefined") {
			if (stackSeries) if (i > 0) lineYPosition = this.lineCtx.prevSeriesY[i - 1][0];
		...
		} else if (stackSeries && i > 0 && typeof series[i][0] === "undefined") {
			for (let s = i - 1; s >= 0; s--) if (series[s][0] !== null && typeof series[s][0] !== "undefined") {
...
var Line = class {
	...
	*/
	draw(series, ctype, seriesIndex, seriesRangeEnd) {
		...
		for (let i = 0; i < series.length; i++) {
			...
			if (w.axisFlags.isXNumeric && w.seriesData.seriesX.length > 0) x = (w.seriesData.seriesX[realIndex][0] - w.globals.minX) / this.xRatio;
			xArrj.push(x);
			const pX = x;
			let pY2;
			const prevX = pX;
			...
			prevY = this.lineHelpers.determineFirstPrevY({
				i,
				realIndex,
				series,
				prevY,
			...
			if (w.config.stroke.curve === "monotoneCubic" && series[i][0] === null) yArrj.push(null);
			else yArrj.push(prevY);
			const pY = prevY;
			let firstPrevY2;
			if (type === "rangeArea") {
			...
			if (type === "rangeArea") {
				...
				for (let s = 0; s < segments; s++) paths.linePaths[s] = rangePaths.linePaths[s + segments] + paths.linePaths[s];
				paths.linePaths.splice(segments);
				paths.pathFromLine = rangePaths.pathFromLine + paths.pathFromLine;
			} else paths.pathFromArea += "z";
			this._handlePaths({
				type,
				realIndex,
				i,
		...
		if (typeof ((_a = w.config.series[0]) == null ? void 0 : _a.zIndex) !== "undefined") allSeries.sort((a, b) => Number(a.node.getAttribute("zIndex")) - Number(b.node.getAttribute("zIndex")));
		if (w.config.chart.stacked) for (let s = allSeries.length - 1; s >= 0; s--) ret.add(allSeries[s]);
		else for (let s = 0; s < allSeries.length; s++) ret.add(allSeries[s]);
		return ret;
	}
	/**
	* @param {any[]} series
	...
	*/
	_initSerieVariables(series, i, realIndex) {
		...
		if (this.zeroY > w.layout.gridHeight || w.config.plotOptions.area.fillTo === "end") this.areaBottomY = w.layout.gridHeight;
		this.categoryAxisCorrection = this.xDivision / 2;
		const seriesItem = w.config.series[realIndex];
		...
		if (w.globals.hasNullValues) {
			...
			if (firstPoint !== null) this.elPointsMain.add(firstPoint);
		}
	...
	_calculatePathsFrom({ type, series, i, realIndex, translationsIndex, prevX, prevY, prevY2 }) {
		...
		if (series[i][0] === null) {
			for (let s = 0; s < series[i].length; s++) if (series[i][s] !== null) {
	...
	_handlePaths({ type, realIndex, i, paths }) {
		...
		if (forecast.count > 0 && type !== "rangeArea") {
			...
			const elForecastMask = graphics.drawRect(forecastCutoff, 0, w.layout.gridWidth, w.layout.gridHeight, 0);
			w.dom.elForecastMask.appendChild(elForecastMask.node);
			const elNonForecastMask = graphics.drawRect(0, 0, forecastCutoff, w.layout.gridHeight, 0);
			w.dom.elNonForecastMask.appendChild(elNonForecastMask.node);
		}
		if (!this.pointsChart) w.globals.delayedElements.push({
		...
		if (type === "area") {
			const pathFill = fill.fillPath({ seriesNumber: realIndex });
		...
		if (w.config.stroke.show && !this.pointsChart) {
			let lineFill = null;
			if (type === "line") lineFill = fill.fillPath({
				seriesNumber: realIndex,
				i
			});
			else if (w.config.stroke.fill.type === "solid") lineFill = w.globals.stroke.colors[realIndex];
			else {
			...
			for (let p = 0; p < paths.linePaths.length; p++) {
				...
				if (type === "rangeArea") pathFill = fill.fillPath({ seriesNumber: realIndex });
	...
	_iterateOverDataPoints({ type, series, iterations, realIndex, translationsIndex, i, x, y, pX, pY, pathsFrom, linePaths, areaPaths, seriesIndex, lineYPosition, xArrj, yArrj, y2Arrj, isRangeStart, seriesRangeEnd }) {
		var _a, _b;
		const w = this.w;
		const graphics = new Graphics(this.w);
		...
		if (Array.isArray(curve)) if (Array.isArray(seriesIndex)) curve = curve[seriesIndex[i]];
		else curve = curve[i];
		let pathState = 0;
		let segmentStartX;
		for (let j = 0; j < iterations; j++) {
			...
			if (w.axisFlags.isXNumeric) {
				...
				if (typeof w.seriesData.seriesX[realIndex][j + 1] === "undefined") sX = w.seriesData.seriesX[realIndex][iterations - 1];
				x = (sX - w.globals.minX) / this.xRatio;
			} else x = x + this.xDivision;
			if (stackSeries) if (i > 0 && w.globals.collapsedSeries.length < w.config.series.length - 1) {
				const prevIndex = (pi) => {
					for (let pii = pi; pii > 0; pii--) if (w.globals.collapsedSeriesIndices.indexOf((seriesIndex == null ? void 0 : seriesIndex[pii]) || pii) > -1) pii--;
					else return pii;
					return 0;
				};
			...
			if (isNull) y = getY(minY, lineYPosition);
			else {
				y = getY(series[i][j + 1], lineYPosition);
				if (type === "rangeArea") y2 = getY(seriesRangeEnd[i][j + 1], lineYPosition);
			}
	...
	_handleMarkersAndLabels({ type, pointsPos, isRangeStart, i, j, realIndex }) {
		...
		const dataLabels = new DataLabels(this.w, this.ctx);
		if (!this.pointsChart) {
			if (w.seriesData.series[i].length > 1) this.elPointsMain.node.classList.add("apexcharts-element-hidden");
		...
		} else this.scatter.draw(this.elSeries, j, {
			realIndex,
			pointsPos,
			zRatio: this.zRatio,
	...
	_createPaths({ type, series, i, j, x, y, xArrj, yArrj, y2, y2Arrj, pX, pY, pathState, segmentStartX, linePath, areaPath, linePaths, areaPaths, curve, isRangeStart }) {
		const graphics = new Graphics(this.w);
		...
		switch (curve) {
			case "monotoneCubic": {
				...
				const getSegmentLengths = (yArr) => {
					...
					if (count > 0) segLens.push(count);
					return segLens;
				};
				const getSegments = (yArr, points) => {
					const segLens = getSegmentLengths(yArr);
					const segments = [];
					for (let i2 = 0, len = 0; i2 < segLens.length; len += segLens[i2++]) segments[i2] = spline.slice(points, len, len + segLens[i2]);
					return segments;
				};
				switch (pathState) {
					case 0:
						if (yAj[j + 1] === null) break;
						pathState = 1;
					case 1: if (!(rangeArea ? xArrj.length === series[i].length : j === series[i].length - 2)) break;
					case 2: {
						...
						if (rangeArea) if (isLowerRangeAreaPath) areaPaths = smoothInputs;
						else smoothInputsLower = areaPaths.reverse();
						let segmentCount = 0;
						...
						getSegments(_yAj, points).forEach((_) => {
							...
							if (isLowerRangeAreaPath) linePath = graphics.move(smoothInputs[_start][0], smoothInputs[_start][1]) + svgPoints;
							else if (rangeArea) linePath = graphics.move(smoothInputsLower[_start][0], smoothInputsLower[_start][1]) + graphics.line(smoothInputs[_start][0], smoothInputs[_start][1]) + svgPoints + graphics.line(smoothInputsLower[_end][0], smoothInputsLower[_end][1]);
							else {
			...
			case "smooth": {
				const length = (x - pX) * .35;
				if (series[i][j] === null) pathState = 0;
				else switch (pathState) {
					case 0:
						segmentStartX = pX;
						if (isLowerRangeAreaPath) linePath = graphics.move(pX, y2Arrj[j]) + graphics.line(pX, pY);
						else linePath = graphics.move(pX, pY);
					...
					case 1:
						if (series[i][j + 1] === null) {
							if (isLowerRangeAreaPath) linePath += graphics.line(pX, y2);
							else linePath += graphics.move(pX, pY);
							...
							areaPath += p;
							if (j >= series[i].length - 2) {
								if (isLowerRangeAreaPath) linePath += graphics.curve(x, y, x, y, x, y2) + graphics.move(x, y2);
				...
				if (series[i][j] === null) pathState = 0;
					...
					case 1:
						if (series[i][j + 1] === null) {
							...
							areaPath += p;
							if (j >= series[i].length - 2) {
								if (isLowerRangeAreaPath) linePath += graphics.line(x, y2);
...
var CircularChartsHelpers = class {
	...
	*/
	drawYAxisTexts(x, y, i, text) {
		...
		return new Graphics(this.w).drawText({
...
var Pie = class {
	...
	*/
	draw(series) {
		...
		const w = this.w;
		const graphics = new Graphics(this.w);
		const elPie = graphics.group({ class: "apexcharts-pie" });
		if (w.globals.noData) return elPie;
		let total = 0;
		for (let k = 0; k < series.length; k++) total += Utils$1.negToZero(series[k]);
		...
		if (w.config.grid.position === "back" && this.chartType === "polarArea") this.drawPolarElements(elPie);
		for (let i = 0; i < series.length; i++) {
		...
		if (w.globals.dataChanged) {
			let prevTotal = 0;
			for (let k = 0; k < w.globals.previousPaths.length; k++) prevTotal += Utils$1.negToZero(w.globals.previousPaths[k]);
			let previousAngle;
		...
		elSeries.attr({ transform: `translate(${this.translateX}, ${this.translateY}) scale(${w.config.plotOptions.pie.customScale})` });
		elSeries.add(elG);
		elPie.add(elSeries);
		if (this.donutDataLabels.show) {
			const dataLabels = this.renderInnerDataLabels(this.dataLabelsGroup, this.donutDataLabels, {
		...
		if (w.config.grid.position === "front" && this.chartType === "polarArea") this.drawPolarElements(elPie);
		return elPie;
	}
	/**
	...
	*/
	drawArcs(sectorAngleArr, series) {
		...
		const graphics = new Graphics(this.w);
		const fill = new Fill(this.w);
		const g = graphics.group({ class: "apexcharts-slices" });
		...
		for (let i = 0; i < sectorAngleArr.length; i++) {
			...
			if (this.chartType === "pie" || this.chartType === "polarArea") {
				labelPosition = Utils$1.polarToCartesian(this.centerX, this.centerY, w.globals.radialSize / 1.25 + w.config.plotOptions.pie.dataLabels.offset, midAngle);
				arcCenter = Utils$1.polarToCartesian(this.centerX, this.centerY, w.globals.radialSize / 2, midAngle);
			} else if (this.chartType === "donut") {
				labelPosition = Utils$1.polarToCartesian(this.centerX, this.centerY, (w.globals.radialSize + this.donutSize) / 2 + w.config.plotOptions.pie.dataLabels.offset, midAngle);
				arcCenter = Utils$1.polarToCartesian(this.centerX, this.centerY, (w.globals.radialSize + this.donutSize) / 2, midAngle);
			}
			...
			if (this.dynamicAnim && w.globals.dataChanged) this.animatePaths(elPath, {
				size: this.sliceSizes[i],
				endAngle,
				startAngle,
				prevStartAngle,
			...
			if (w.config.plotOptions.pie.expandOnClick && this.chartType !== "polarArea") elPath.node.addEventListener("mouseup", this.pieClicked.bind(this, i));
			if (typeof w.interact.selectedDataPoints[0] !== "undefined" && w.interact.selectedDataPoints[0].indexOf(i) > -1) this.pieClicked(i);
			if (w.config.dataLabels.enabled) {
				...
				if (angle !== 0 && w.config.plotOptions.pie.dataLabels.minAngleToShowLabel < sectorAngleArr[i]) {
					const formatter = w.config.dataLabels.formatter;
					if (formatter !== void 0) text = formatter(w.globals.seriesPercent[i][0], {
						seriesIndex: i,
						w
					});
					const foreColor = w.globals.dataLabels.style.colors[i];
					const elPieLabelWrap = graphics.group({ class: `apexcharts-datalabels` });
	...
	*/
	addListeners(elPath, dataLabels) {
		...
		elPath.node.addEventListener("mouseleave", this.revertDataLabelsInner.bind(this));
	...
	*/
	animatePaths(el, opts) {
		...
		if (angle === this.fullAngle) angle = this.fullAngle - .01;
	...
	*/
	animateArc(el, fromStartAngle, toStartAngle, angle, prevAngle, opts) {
		...
		if (opts.dur !== 0) el.animate(opts.dur, opts.animBeginArr[opts.i]).after(
			/** @this {any} */
			function() {
	...
	*/
	pieClicked(i) {
		...
		const elPath = w.dom.Paper.findOne(`.apexcharts-${me.chartType.toLowerCase()}-slice-${i}`);
	...
	*/
	getChangedPath(prevStartAngle, prevEndAngle) {
		let path = "";
		if (this.dynamicAnim && this.w.globals.dataChanged) path = this.getPiePath({
			me: this,
	...
	getPiePath({ me, startAngle, angle, size }) {
		...
		if (Math.ceil(endDeg) >= this.fullAngle + this.w.config.plotOptions.pie.startAngle % this.fullAngle) endDeg = this.fullAngle + this.w.config.plotOptions.pie.startAngle % this.fullAngle - .01;
		...
		const startInner = Utils$1.polarToCartesian(me.centerX, me.centerY, me.donutSize, endDeg);
		const endInner = Utils$1.polarToCartesian(me.centerX, me.centerY, me.donutSize, startDeg);
		...
		else if (me.chartType === "pie" || me.chartType === "polarArea") path = [
			...pathBeginning,
			"L",
			me.centerX,
			me.centerY,
	...
	*/
	renderInnerDataLabels(dataLabelsGroup, dataLabelsConfig, opts) {
		...
		if (dataLabelsConfig.name.color === void 0) labelColor = w.globals.colors[0];
		...
		if (dataLabelsConfig.value.color === void 0) valueColor = w.config.chart.foreColor;
		...
		} else if (w.seriesData.series.length === 1) {
			val = lbFormatter(w.seriesData.series[0], w);
			name2 = w.seriesData.seriesNames[0];
		}
		if (name2) name2 = dataLabelsConfig.name.formatter(name2, dataLabelsConfig.total.show, w);
	...
	*/
	printInnerLabels(labelsConfig, name2, val, el) {
		const w = this.w;
		let labelColor;
		if (el) if (labelsConfig.name.color === void 0) labelColor = w.globals.colors[parseInt(el.parentNode.getAttribute("rel"), 10) - 1];
		else labelColor = labelsConfig.name.color;
		else if (w.seriesData.series.length > 1 && labelsConfig.total.show) labelColor = labelsConfig.total.color;
		...
		if (elLabel !== null) elLabel.textContent = name2;
		if (elValue !== null) elValue.textContent = val;
		if (elLabel !== null) {
			const elLabelEl = elLabel;
	...
	*/
	printDataLabelsInner(el, dataLabelsConfig) {
		...
		if (w.seriesData.series.length > 1) this.printInnerLabels(dataLabelsConfig, name2, val, el);
		const dataLabelsGroup = w.dom.baseEl.querySelector(".apexcharts-datalabels-group");
		if (dataLabelsGroup !== null) {
			const dataLabelsGroupEl = dataLabelsGroup;
	...
	*/
	drawSpokes(parent) {
		...
		for (let i = 0; i < w.seriesData.series.length; i++) spokes.push(Utils$1.polarToCartesian(this.centerX, this.centerY, w.globals.radialSize, w.config.plotOptions.pie.startAngle + angleDivision * i));
		spokes.forEach((p, i) => {
	...
	revertDataLabelsInner() {
		const w = this.w;
		if (this.donutDataLabels.show) {
			const dataLabelsGroup = w.dom.Paper.findOne(`.apexcharts-datalabels-group`);
			const dataLabels = this.renderInnerDataLabels(dataLabelsGroup, this.donutDataLabels, {
				hollowSize: this.donutSize,
				centerX: this.centerX,
				centerY: this.centerY,
				opacity: this.donutDataLabels.show
			});
			w.dom.Paper.findOne(".apexcharts-radialbar, .apexcharts-pie").add(dataLabels);
		}
	}
};
var Radar = class {
	...
	*/
	constructor(w, ctx) {
		...
		if (w.config.xaxis.labels.show) this.size = this.size - w.layout.xAxisLabelsWidth / 1.75;
		if (w.config.plotOptions.radar.size !== void 0) this.size = w.config.plotOptions.radar.size;
	...
	draw(series) {
		...
		series.forEach((s, i) => {
			...
			s.forEach((dv, j) => {
				...
				dv = dv - this.minValue;
				if (this.isLog) dv = this.coreUtils.getLogVal(this.logBase, dv, 0);
			...
			elPointsMain = this.graphics.group({ class: "apexcharts-series-markers-wrap apexcharts-element-hidden" });
			...
			let pathFrom = null;
			if (w.globals.previousPaths.length > 0) pathFrom = this.getPreviousPath(i);
			for (let p = 0; p < paths.linePathsTo.length; p++) {
				...
				const pathFill = fill.fillPath({ seriesNumber: i });
			...
			s.forEach((sj, j) => {
				const opts = new Markers(this.w, this.ctx).getMarkerConfig({
					cssClass: "apexcharts-marker",
					seriesIndex: i,
					dataPointIndex: j
				});
				const point = this.graphics.drawMarker(dataPointsPos[j].x, dataPointsPos[j].y, opts);
				point.attr("rel", j);
				point.attr("j", j);
				point.attr("index", i);
				point.node.setAttribute("default-marker-size", opts.pSize);
				const elPointsWrap = this.graphics.group({ class: "apexcharts-series-markers" });
	...
	*/
	drawPolygons(opts) {
		...
		for (let i = 0; i < layers; i++) radiusSizes[i] = layerDis * i;
		radiusSizes.reverse();
		...
		radiusSizes.forEach((radiusSize, r) => {
			...
			polygon.forEach((p, i) => {
				...
				if (i === 0) this.yaxisLabelsTextsPos.push({
					x: p.x,
					y: p.y
				});
				string += p.x + "," + p.y + " ";
		...
		if (w.config.yaxis[0].show) this.yaxisLabelsTextsPos.forEach((p, i) => {
	...
	drawXAxisTexts() {
		...
		const elXAxisWrap = this.graphics.group({ class: "apexcharts-xaxis" });
...
var Radial = class extends Pie {
	...
	*/
	draw(series) {
		const w = this.w;
		const graphics = new Graphics(this.w);
		const ret = graphics.group({ class: "apexcharts-radialbar" });
		...
		if (this.radialDataLabels.value.show) {
			const offset = Math.max(this.radialDataLabels.value.offsetY, this.radialDataLabels.name.offsetY);
	...
	*/
	drawTracks(opts) {
		const w = this.w;
		const graphics = new Graphics(this.w);
		const g = graphics.group({ class: "apexcharts-tracks" });
		const filters = new Filters(this.w);
		const fill = new Fill(this.w);
		const strokeWidth = this.getStrokeWidth(opts);
		opts.size = opts.size - strokeWidth / 2;
		for (let i = 0; i < opts.series.length; i++) {
			const elRadialBarTrack = graphics.group({ class: "apexcharts-radialbar-track apexcharts-track" });
			...
			if (Math.abs(endAngle) + Math.abs(startAngle) >= 360) endAngle = 360 - Math.abs(this.startAngle) - .1;
			const elPath = graphics.drawPath({
				d: "",
				stroke: pathFill,
	...
	*/
	drawArcs(opts) {
		...
		if (w.config.plotOptions.radialBar.hollow.image !== void 0) hollowFillID = this.drawHollowImage(opts, g, hollowSize, hollowFillID);
		...
		let shown = 1;
		if (!this.radialDataLabels.total.show && w.seriesData.series.length > 1) shown = 0;
		let dataLabels = null;
		...
		for (let i = reverseLoop ? opts.series.length - 1 : 0; reverseLoop ? i >= 0 : i < opts.series.length; reverseLoop ? i-- : i++) {
			...
			if (Math.abs(endAngle) + Math.abs(startAngle) > 360) endAngle = endAngle - .01;
			if (Math.abs(prevEndAngle) + Math.abs(prevStartAngle) > 360) prevEndAngle = prevEndAngle - .01;
			...
			const radialArcCenter = Utils$1.polarToCartesian(opts.centerX, opts.centerY, opts.size, radialMidAngle);
			Graphics.setAttrs(elPath.node, {
				"data:angle": angle,
			...
			if (this.barLabels.enabled) {
				const barStartCords = Utils$1.polarToCartesian(opts.centerX, opts.centerY, opts.size, startAngle);
				const text = this.barLabels.formatter(w.seriesData.seriesNames[i], {
					seriesIndex: i,
					w
				});
				const classes = ["apexcharts-radialbar-label"];
				if (!this.barLabels.onClick) classes.push("apexcharts-no-click");
	...
	*/
	drawHollow(opts) {
		const circle = new Graphics(this.w).drawCircle(opts.size * 2);
		circle.attr({
	...
	*/
	onBarLabelClick(e) {
		...
		const w = this.w;
		if (legendClick) legendClick(w.seriesData.seriesNames[seriesIndex], {
			w,
			seriesIndex
		});
	}
...
var RangeBar = class extends Bar {
	...
	*/
	draw(series, seriesIndex) {
		...
		this.barHelpers.initVariables(series);
		const ret = graphics.group({ class: "apexcharts-rangebar-series apexcharts-plot-series" });
		for (let i = 0; i < series.length; i++) {
			let x, y;
			const realIndex = w.globals.comboCharts ? seriesIndex[i] : i;
			const { columnGroupIndex } = this.barHelpers.getGroupIndex(realIndex);
			...
			const initPositions = this.barHelpers.initialPositions(realIndex);
			const { y: initY, zeroW, x: initX, zeroH } = initPositions;
			...
			const elGoalsMarkers = graphics.group({ class: "apexcharts-rangebar-goals-markers" });
			for (let j = 0; j < w.globals.dataPoints; j++) {
				const strokeWidth = this.barHelpers.getStrokeWidth(i, j, realIndex);
				...
				if (w.config.plotOptions.bar.rangeBarGroupRows) seriesLen = 1;
				if (typeof ((_e = w.config.series[i].data) == null ? void 0 : _e[j]) === "undefined") break;
				if (this.isHorizontal) {
					barYPosition = y + barHeight * this.visibleI;
					const srty = (yDivision - barHeight * seriesLen) / 2;
					if ((_g = (_f = w.config.series[i].data) == null ? void 0 : _f[j]) == null ? void 0 : _g.x) {
					...
					if (w.axisFlags.isXNumeric) x = (w.seriesData.seriesX[i][j] - w.globals.minX) / this.xRatio - barWidth / 2;
					barXPosition = x + barWidth * this.visibleI;
					const srtx = (xDivision - barWidth * seriesLen) / 2;
					if ((_i = (_h = w.config.series[i].data) == null ? void 0 : _h[j]) == null ? void 0 : _i.x) {
						const positions = this.detectOverlappingBars({
							i,
							j,
							barXPosition,
	...
	detectOverlappingBars({ i, j, barYPosition, barXPosition, srty, srtx, barHeight, barWidth, yDivision, xDivision, initPositions }) {
		var _a, _b, _c, _d;
		const w = this.w;
		let overlaps = [];
		const rangeName = (_b = (_a = w.config.series[i].data) == null ? void 0 : _a[j]) == null ? void 0 : _b.rangeName;
		const x = (_d = (_c = w.config.series[i].data) == null ? void 0 : _c[j]) == null ? void 0 : _d.x;
		const labelX = Array.isArray(x) ? x.join(" ") : x;
		const rowIndex = w.labelData.labels.map((_) => Array.isArray(_) ? _.join(" ") : _).indexOf(labelX);
		const overlappedIndex = w.rangeData.seriesRange[i].findIndex((tx) => {
			var _a2;
		...
		if (this.isHorizontal) {
			...
			if (rowIndex > -1 && !w.labelData.timescaleLabels.length) if (w.config.plotOptions.bar.rangeBarGroupRows) barXPosition = srtx + xDivision * rowIndex;
			else barXPosition = srtx + barWidth * this.visibleI + xDivision * rowIndex;
			if (overlappedIndex > -1 && !w.config.plotOptions.bar.rangeBarOverlap) {
				overlaps = Array.from(
					/** @type {any} */
					w.rangeData.seriesRange[i][overlappedIndex].overlaps
	...
	drawRangeColumnPaths({ indexes, x, xDivision, barWidth, barXPosition, zeroH }) {
		var _a, _b;
		const w = this.w;
		...
		if (typeof ((_a = this.series[i]) == null ? void 0 : _a[j]) === "undefined" || ((_b = this.series[i]) == null ? void 0 : _b[j]) === null) y1 = zeroH;
		else {
			y1 = zeroH - y1 / yRatio;
			y2 = zeroH - y2 / yRatio;
		}
	...
	*/
	preventBarOverflow(val) {
		const w = this.w;
		if (val < 0) val = 0;
		if (val > w.layout.gridWidth) val = w.layout.gridWidth;
		return val;
	}
	/** @param {{indexes: any, y: any, y1: any, y2: any, yDivision: any, barHeight: any, barYPosition: any, zeroW: any}} opts */
	drawRangeBarPaths({ indexes, y, y1, y2, yDivision, barHeight, barYPosition, zeroW }) {
		const w = this.w;
...
function normalize(data, area) {
	...
	for (let i = 0; i < data.length; i++) result[i] = data[i] * multiplier;
	return result;
}
...
function improvesRatio(rowLen, rowMin, rowMax, rowSum, nextNode, length) {
	...
	return calculateRatio(rowMin, rowMax, rowSum, length) >= calculateRatio(Math.min(rowMin, nextNode), Math.max(rowMax, nextNode), rowSum + nextNode, length);
}
...
function squarify(data, xoffset, yoffset, width, height) {
	...
	if (rowLen > 0) emitCoordinates(coords, row, rowLen, rowSum, xoffset, yoffset, width, height);
	return coords;
}
function generate(data, width, height) {
	...
	for (let i = 0; i < n; i++) {
		let s = 0;
		const series = data[i];
		for (let j = 0; j < series.length; j++) s += series[j];
		sums[i] = s;
	}
...
var TreemapChart = class {
	...
	*/
	draw(series) {
		...
		const graphics = new Graphics(this.w, this.ctx);
		const fill = new Fill(this.w);
		const ret = graphics.group({ class: "apexcharts-treemap" });
		if (w.globals.noData) return ret;
		const ser = [];
		series.forEach((s) => {
			const d = s.map((v) => {
		...
		TreemapSquared.generate(ser, w.layout.gridWidth, w.layout.gridHeight).forEach((node, i) => {
			var _a;
			const elSeries = graphics.group({
			...
			if (w.config.chart.dropShadow.enabled) {
				const shadow = w.config.chart.dropShadow;
				new Filters(this.w).dropShadow(ret, shadow, i);
			}
			const elDataLabelWrap = graphics.group({ class: "apexcharts-data-labels" });
			const bounds = {
				xMin: Infinity,
				yMin: Infinity,
				xMax: -Infinity,
			...
			node.forEach((r, j) => {
				...
				const colorProps = this.helpers.getShadeColor(w.config.chart.type, i, j, this.negRange);
				...
				const elRect = graphics.drawRect(x1, y1, x2 - x1, y2 - y1, w.config.plotOptions.treemap.borderRadius, "#fff", 1, this.strokeWidth, w.config.plotOptions.treemap.useFillColorAsStroke ? color : w.globals.stroke.colors[i]);
				elRect.attr({
					cx: x1,
					cy: y1,
					index: i,
				...
				if (w.globals.dataChanged) {
					...
					if (this.dynamicAnim.enabled && w.globals.shouldAnimate) {
						speed = this.dynamicAnim.speed;
						if (w.globals.previousPaths[i] && w.globals.previousPaths[i][j] && w.globals.previousPaths[i][j].rect) fromRect = w.globals.previousPaths[i][j].rect;
						this.animateTreemap(elRect, fromRect, toRect, speed);
					}
				}
				...
				if (w.config.plotOptions.treemap.dataLabels.format === "truncate") {
					...
					formattedText = this.truncateLabels(String(formattedText), fontSize, x1, y1, x2, y2);
				}
				let dataLabels = null;
				if (w.seriesData.series[i][j]) dataLabels = this.helpers.calculateDataLabels({
					text: formattedText,
					x: (x1 + x2) / 2,
				...
				if (w.config.dataLabels.enabled && dataLabels) this.rotateToFitLabel(dataLabels, fontSize, formattedText, x1, y1, x2, y2);
				elSeries.add(elRect);
				if (dataLabels !== null) elSeries.add(dataLabels);
			});
			...
			if (w.config.series.length > 1 && seriesTitle && seriesTitle.show) {
				...
				if (sName && bounds.xMin < Infinity && bounds.yMin < Infinity) {
					const { offsetX, offsetY, borderColor, borderWidth, borderRadius, style } = seriesTitle;
					...
					const elLabelText = graphics.drawText({
						...
						y: labelY + padding.top + ((_a = textSize == null ? void 0 : textSize.height) != null ? _a : 0) * .75,
						text: sName,
	...
	*/
	getFontSize(coordinates) {
		...
		function totalLabelLength(arr) {
			let i, total = 0;
			if (Array.isArray(arr[0])) for (i = 0; i < arr.length; i++) total += totalLabelLength(arr[i]);
			else for (i = 0; i < arr.length; i++) total += arr[i].length;
			return total;
		}
		function countLabels(arr) {
			let i, total = 0;
			if (Array.isArray(arr[0])) for (i = 0; i < arr.length; i++) total += countLabels(arr[i]);
			else for (i = 0; i < arr.length; i++) total += 1;
			return total;
		}
		const averagelabelsize = totalLabelLength(this.labels) / countLabels(this.labels);
		function fontSize(width, height) {
			const area = width * height;
			return Math.min(Math.pow(area, .5) / averagelabelsize, parseInt(w.config.dataLabels.style.fontSize, 10));
		}
	...
	*/
	truncateLabels(text, fontSize, x1, y1, x2, y2) {
		const graphics = new Graphics(this.w);
		const labelMaxWidth = graphics.getTextRects(text, String(fontSize)).width + this.w.config.stroke.width + 5 > x2 - x1 && y2 - y1 > x2 - x1 ? y2 - y1 : x2 - x1;
		const truncatedText = graphics.getTextBasedOnMaxWidth({
			text,
			maxWidth: labelMaxWidth,
			fontSize
		});
		if (text.length !== truncatedText.length && labelMaxWidth / fontSize < 5) return "";
...
//#endregion
//#region node_modules/react-apexcharts/dist/react-apexcharts.esm.js
...
function l(r, e) {
	var t = f({}, r);
	return p(r) && p(e) && Object.keys(e).forEach((function(n) {
		p(e[n]) ? t[n] = n in r ? l(r[n], e[n]) : e[n] : t[n] = e[n];
	})), t;
}
function y(r, e) {
	var t = arguments.length > 2 && void 0 !== arguments[2] ? arguments[2] : /* @__PURE__ */ new WeakSet();
	if (r === e) return !0;
	if ("object" !== a(r) || null === r || "object" !== a(e) || null === e) return !1;
	if (t.has(r) || t.has(e)) return !0;
	t.add(r), t.add(e);
	var n = Object.keys(r), o = Object.keys(e);
	if (n.length !== o.length) return !1;
	for (var i = 0, u = n; i < u.length; i++) {
		var c = u[i];
		if (!o.includes(c) || !y(r[c], e[c], t)) return !1;
	}
	return !0;
}
var b = [
...
function h(o) {
	var i = o.type, c = void 0 === i ? "line" : i, a = o.width, p = void 0 === a ? "100%" : a, h = o.height, d = void 0 === h ? "auto" : h, v = o.series, O = o.options, g = o.chartRef, m = function(r, e) {
		if (null == r) return {};
		...
		if (Object.getOwnPropertySymbols) {
			...
			for (n = 0; n < i.length; n++) t = i[n], e.includes(t) || {}.propertyIsEnumerable.call(r, t) && (o[t] = r[t]);
		}
		return o;
	}(o, s), j = (0, import_react.useRef)(null), w = (0, import_react.useRef)(null), P = (0, import_react.useRef)(null), S = g || P, E = function() {
		return l(O, {
			chart: {
				type: c,
				height: d,
	...
	(0, import_react.useEffect)((function() {
		return S.current = new ApexCharts(j.current, E()), S.current.render(), w.current = O, function() {
			S.current && "function" == typeof S.current.destroy && S.current.destroy();
		};
	}), []), (0, import_react.useEffect)((function() {
		if (S.current && S.current.w) {
			var r = !y(S.current.w.config.series, v), e = !y(w.current, O) || d !== S.current.opts.chart.height || p !== S.current.opts.chart.width;
	...
	return import_react.createElement("div", u({ ref: j }, D));
}
h.propTypes = {
	type: import_prop_types.default.string.isRequired,
	series: import_prop_types.default.array.isRequired,
	...
	chartRef: import_prop_types.default.shape({ current: import_prop_types.default.any })
};
//#endregion
export { h as default };
