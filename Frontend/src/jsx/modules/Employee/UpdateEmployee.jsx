...
import { getEmployeeById, updateEmployee } from "./APIS";
...
const UpdateEmployee = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ✅ Create FormData with correct backend fields
      const data = new FormData();
      Object.entries(formData).forEach(([key, value]) => {
        if (value !== null && value !== "") {
          if (key === "photo") data.append("user_img", value);
          else if (key === "phone") data.append("phone_no", value);
          else if (key === "ctc") data.append("ctc", parseFloat(value));
          else if (key === "empName") data.append("username", value);
          else if (key === "empEmail") data.append("email", value);
          else if (key === "reportingHead") data.append("reporting_head", value);
      ...
      if (ok) {
        ...
        navigate("/employee-List", { state: { updated: true } }); // trigger list refresh
      } else {
          ...
          <Card.Body>
            <form onSubmit={handleSubmit}>
              <div className="row">
                ...
                <div className="col-lg-6 mb-3">
                  <label>Email *</label>
                  <input type="email" className="form-control" name="empEmail" value={formData.empEmail} onChange={handleChange} required />
                </div>
                ...
                <div className="col-lg-6 mb-3">
                  ...
                  <div className="input-group">
                    <input type={showPassword ? "text" : "password"} className="form-control" name="password" value={formData.password} onChange={handleChange} placeholder="Enter new password" />
                ...
                <div className="col-lg-6 mb-3">
                  ...
                  <input type="text" className="form-control" name="reportingHead" value={formData.reportingHead} onChange={handleChange} />
                </div>
              ...
              <div className="text-end mt-3">
                <button className="btn btn-secondary me-2" type="button" onClick={() => navigate("/employee-list")}>Cancel</button>
                <button className="btn btn-primary" type="submit" disabled={loading}>{loading ? "Updating..." : "Update Employee"}</button>
              </div>
            </form>
          </Card.Body>
        </Card>
