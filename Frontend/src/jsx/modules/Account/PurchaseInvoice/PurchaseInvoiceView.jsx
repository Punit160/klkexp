  ...
  return (
    <>
          ...
          <div className="d-flex align-items-center gap-3">
            ...
            <div>
              <div className="d-flex align-items-center gap-2 flex-wrap">
                <h5 className="mb-0 fw-bold">{data?.PurchaseNo || "Purchase Invoice"}</h5>
              ...
              <small className="text-muted">
                {data?.InvoiceType || "Tax Invoice"} · {data?.PurchaseDate || "—"}
              </small>
            </div>
          </div>
          <div className="d-flex gap-2 flex-wrap">
            {canApprove && (
              <button type="button" className="btn btn-success btn-sm" onClick={onApprove} disabled={actionBusy}>
                ...
                {actionBusy ? "Approving..." : "Approve Purchase"}
              </button>
            )}
            {canTallyPush && (
      ...
      <Row className="g-3 mb-3">
        <Col xl={3} md={6}>
          <SummaryTile label="Vendor" value={data?.VendorName} sub={data?.Vendorgstin ? `GSTIN: ${data.Vendorgstin}` : ""} />
        </Col>
        <Col xl={3} md={6}>
          <SummaryTile label="Buyer" value={data?.CompanyName} sub={data?.CompanyGstin ? `GSTIN: ${data.CompanyGstin}` : ""} />
        </Col>
        <Col xl={3} md={6}>
          <SummaryTile label="Consignee" value={data?.ConsigneeName || "Same as buyer"} />
        </Col>
        <Col xl={3} md={6}>
          <SummaryTile
            label="Grand Total"
            value={formatMoney(data?.PurchaseAmount)}
            sub={data?.PONo ? `PO: ${data.PONo}` : ""}
          />
        </Col>
      </Row>

      <PurchaseInvoicePreview data={data} />
