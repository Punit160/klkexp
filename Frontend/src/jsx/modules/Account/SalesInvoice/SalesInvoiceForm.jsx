...
import {
  ...
  mapCompanyToSalesConsigneeFields,
...
const PARTY_FIELD_CONFIG = {
  ...
  customer: [
    ...
    { label: "GSTIN/UIN", key: "customergstin" },
...
const SalesInvoiceForm = ({ invoiceId, initialData, onDataChange, onClose, onSaved }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    // ---- Seller (Our Company) Details ----
    SellerCompanyId: initialData?.SellerCompanyId || "",
  ...
  const loadSellerBanks = async (companyId) => {
    if (!companyId) {
      setSellerBanks([]);
      return;
    }
    try {
      ...
      setSellerBanks(getCompanyBankAccounts(company));
      setCompanies((prev) => {
  ...
  useEffect(() => {
    if (formData.SellerCompanyId) {
      loadSellerBanks(formData.SellerCompanyId);
    }
  ...
  const handleCompanySelect = (type, companyId) => {
    if (!companyId) {
      if (type === "seller") {
        setFormData((prev) => ({ ...prev, ...SELLER_CLEAR }));
        setSellerBanks([]);
      } else if (type === "consignee") {
        setFormData((prev) => ({ ...prev, ...CONSIGNEE_CLEAR }));
      } else {
        setFormData((prev) => ({ ...prev, ...CUSTOMER_CLEAR }));
      }
      return;
    }
    ...
    if (type === "seller") {
      setFormData((prev) => ({
        ...prev,
        ...mapCompanyToSellerFields(company),
        ...BANK_FIELDS_CLEAR,
      }));
      loadSellerBanks(companyId);
    } else if (type === "consignee") {
      setFormData((prev) => ({ ...prev, ...mapCompanyToSalesConsigneeFields(company) }));
    } else {
      setFormData((prev) => ({ ...prev, ...mapCompanyToCustomerFields(company) }));
    }
  };

  const handleBankAccountSelect = (bankAccountId) => {
    const bank = sellerBanks.find((b) => String(b.id) === String(bankAccountId));
    setFormData((prev) => ({
      ...prev,
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.SellerCompanyId || !formData.InvoiceNo) {
      toast.error("Please select seller and enter Invoice No");
      return;
    }

    if (sellerBanks.length && !formData.BankAccountId) {
      toast.error("Please select a seller bank account");
      return;
    }

    if (!billItems.some((item) => item.productId && item.quantity && item.rate)) {
    ...
    try {
      setSubmitting(true);
      const payload = mapFormToPayload(formData, billItems, gstDetails);

      const result = invoiceId
        ? await updateSales(invoiceId, payload)
        : await createSales(payload);
      toast.success(invoiceId ? "Sales invoice updated successfully" : "Sales invoice created successfully");
      if (onSaved) onSaved(invoiceId || result?.data?.id);
    } catch (error) {
  ...
  const handleApplyScan = (scanned) => {
    ...
    setBillItems(scanned.items);
    setGstDetails(scanned.gstDetails);
    if (scanned.formFields.SellerCompanyId) {
      loadSellerBanks(scanned.formFields.SellerCompanyId);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="pi-purchase-form">
      <AiDocumentScanner
        ...
        badge="Auto-fill sales invoice"
        description="Upload a sales invoice or take a photo — AI fills parties, items, GST, and dispatch details."
        scanFile={(file) => scanAndMapSalesBill(file, companies, products)}
        ...
        renderPreview={(preview) => (
          <div className="row g-2 small">
            ...
            <div className="col-md-3">
              ...
              <div className="fw-medium text-truncate">{preview.summary?.customerName || "—"}</div>
            </div>
            <div className="col-md-3">
        ...
        <div className="row mb-0">
          <div className="col-lg-4">
            {renderPartySection(
              "Seller (Our Company)",
              "seller",
              formData.SellerCompanyId,
              !!formData.SellerCompanyId,
              PARTY_FIELD_CONFIG.seller,
              sellerBanks
            )}
          </div>
          <div className="col-lg-4">
      ...
      <div className="pi-form-actions sticky-bottom account-surface border rounded shadow-sm p-3 mt-2">
        <div className="text-end">
          ...
          <button type="submit" className="btn btn-primary px-4" disabled={submitting}>
            {submitting ? "Saving..." : "Save Sales Invoice"}
          </button>
        </div>
      </div>
    </form>
