...
const PurchaseInvoice = () => {
  ...
  const {
    ...
    keys: ["PurchaseNo", "VendorName", "status"],
    itemsPerPage: 100,
  });

  const handleDelete = (id) => {
    if (!window.confirm("Are you sure you want to delete this purchase invoice?")) return;
    alert(`(Static demo) Purchase invoice ${id} would be deleted here.`);
  };
  ...
  return (
    <>
      <PageTitle activeMenu="Purchase Invoice Management" motherMenu="Tally" />
                  ...
                  <Card.Header className="d-flex align-items-center justify-content-between flex-wrap gap-3 bg-white">
                    ...
                    <div className="d-flex align-items-center gap-2 flex-wrap flex-lg-nowrap ms-auto">
                      ...
                      <button
                        ...
                        onClick={openAddForm}
                      >
                        <i className="fa fa-plus"></i> New Purchase Invoice
                      </button>
                    </div>
                  </Card.Header>
                </Card.Body>
            ...
            <Col lg={12}>
              <Card className="border-0 shadow-sm">
                <Card.Body className="pt-0">
                  <Table responsive className="pi-table text-nowrap align-middle mb-0">
                    <thead>
                      <tr>
                        <th>Sno</th>
                        <th>Purchase No</th>
                      ...
                      {filteredData.length > 0 ? (
                        filteredData.map((note, index) => (
                          <tr key={note.id || index}>
                            ...
                            <td className="fw-semibold">{note.PurchaseNo || "N/A"}</td>
                            <td>{note.PurchaseDate || "N/A"}</td>
                            <td>{note.PONo || "N/A"}</td>
                            <td>{note.VendorName || "N/A"}</td>
                            <td className="fw-bold text-success">
                              {note.PurchaseAmount != null
                                ? `\u20b9${note.PurchaseAmount.toLocaleString("en-IN")}`
                                : "N/A"}
                            </td>
                            ...
                            <td>
                              <Badge
                                ...
                                className="pi-status-badge rounded-pill"
                              >
        ...
        <Row>
          ...
          <Col lg={8}>
            <Card className="border-0 shadow-sm">
              <Card.Header className="d-flex align-items-center gap-2 bg-white">
                <button className="btn btn-light pi-action-btn" onClick={closeForm} title="Back to list">
          ...
          <Col lg={4}>
            <div className="pi-sticky-panel">
              <PurchaseInvoicePreview data={liveData} isEdit={!!editId} />
            </div>
          </Col>
        </Row>
      )}
      ...
      <style>{`
    .pi-sticky-panel { position: sticky; top: 85px; }
        @media (max-width: 991px) { .pi-sticky-panel { position: static; margin-top: 1.5rem; } }
      `}
      </style>
    </>
  );
