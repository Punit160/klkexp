...
const DeliveryChallan = () => {
  ...
  const {
    ...
    keys: ["Challanno", "CustomerName", "status"],
    itemsPerPage: 100,
  });

  const handleDelete = (id) => {
    if (!window.confirm("Are you sure you want to delete this delivery challan?")) return;
    alert(`(Static demo) Delivery challan ${id} would be deleted here.`);
  };
    ...
    setLiveData(
      record
        ? {
          ...
          Challanamount: record.Challanamount,
  ...
  const filteredData = paginatedData.filter((c) => {
    ...
    const matchesDate = dateFilter ? c.Challandate === dateFilter : true;
    return matchesStatus && matchesDate;
  });

  return (
    <>
      <PageTitle activeMenu="Delivery Challan Management" motherMenu="Tally" />
              ...
              <Card className="border-0 shadow-sm rounded">
                <Card.Body>
                  <Card.Header className="d-flex align-items-center justify-content-between flex-wrap gap-3 bg-white">
                    <Card.Title className="mb-0 flex-shrink-0 fw-bold">Challan List</Card.Title>
                    ...
                    <div className="d-flex align-items-center gap-2 flex-wrap flex-lg-nowrap ms-auto">
                      ...
                      <div className="flex-shrink-0">
                        <TableExportActions
                          ...
                          fileName="Delivery_Challan_List"
                        />
                      </div>
                      ...
                      <button
                        ...
                        onClick={openAddForm}
                      >
                        <i className="fa fa-plus"></i> New Delivery Challan
                      </button>
                    </div>
                  </Card.Header>
                </Card.Body>
            ...
            <Col lg={12}>
              <Card className="border-0 shadow-sm">
                <Card.Body className="pt-0">
                  <Table responsive className="dc-table text-nowrap align-middle mb-0">
                    <thead>
                      <tr>
                        <th>Sno</th>
                        <th>Challan No</th>
                      ...
                      {filteredData.length > 0 ? (
                        filteredData.map((note, index) => (
                          <tr key={note.id || index}>
                            ...
                            <td className="fw-semibold">{note.Challanno || "N/A"}</td>
                            <td>{note.Challandate || "N/A"}</td>
                            <td>{note.CustomerName || "N/A"}</td>
                            <td className="fw-bold text-success">
                              {note.Challanamount != null
                                ? `\u20b9${note.Challanamount.toLocaleString("en-IN")}`
                                : "N/A"}
                            </td>
                            ...
                            <td>
                              <Badge
                                ...
                                className="dc-status-badge rounded-pill"
                              >
      ...
      {view === "form" && (
        <Row>
          ...
          <Col lg={8}>
            <Card className="border-0 shadow-sm">
              <Card.Header className="d-flex align-items-center gap-2 bg-white">
                <button className="btn btn-light dc-action-btn" onClick={closeForm} title="Back to list">
          ...
          {/* Live preview — col-4 */}
          <Col lg={4}>
            <div className="dc-sticky-panel">
              <DeliveryChallanPreview data={liveData} isEdit={!!editId} />
            </div>
          </Col>
        </Row>
      )}
      ...
      <style>{`
    .dc-sticky-panel { position: sticky; top: 85px; }
        @media (max-width: 991px) { .dc-sticky-panel { position: static; margin-top: 1.5rem; } }
      `}
      </style>
    </>
  );
